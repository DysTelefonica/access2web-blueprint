Attribute VB_Name = "modFrmNCProyectoControlEficaciaAltaHelper"
Option Compare Database
Option Explicit

' =============================================================================
' Form helper for Form_FormNCProyectoControlEficaciaAlta (slice 10 of form-thin-helper-refactor).
'
' This helper owns the 1 message-class decision that previously lived inline
' in Form_FormNCProyectoControlEficaciaAlta.cls event handlers:
...
' Slice:    10 of 25 (ControlEficaciaAlta pair -- NCProyecto side; companion is
'           modFrmNCAuditoriaControlEficaciaAltaHelper.bas on NCAuditoria side).
' =============================================================================

' ----- Message contracts (byte-for-byte match with Test_modFrmNCProyectoControlEficaciaAltaHelper.bas) -----
...
'         End If
'         pregunta = modFrmNCProyectoControlEficaciaAltaHelper_RenderError(m_Error, Err.Number, , m_HelperErr)
'
...
' -----------------------------------------------------------------------------
Public Function modFrmNCProyectoControlEficaciaAltaHelper_RenderError( _
    ...
    If Len(Trim$(p_ErrorText)) = 0 Then
        modFrmNCProyectoControlEficaciaAltaHelper_RenderError = PROMPT_ATOM
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ...
        Else
            style = vbCritical
        End If
        modFrmNCProyectoControlEficaciaAltaHelper_RenderError = MsgBox(p_ErrorText, style, MSG_ERROR_TITLE)
        Exit Function
    End If

    ' Test simulating user response.
    modFrmNCProyectoControlEficaciaAltaHelper_RenderError = p_PromptResult
    Exit Function

EH:
    p_Error = "modFrmNCProyectoControlEficaciaAltaHelper_RenderError: " & Err.Description
    modFrmNCProyectoControlEficaciaAltaHelper_RenderError = vbOK
End Function
