Attribute VB_Name = "modFrmNCAuditoriaDocumentosHelper"
Option Compare Database
Option Explicit

' =============================================================================
' Form helper for Form_FormNCAuditoriaDocumentos (slice 6a of form-thin-helper-refactor).
'
' This helper owns the 3 message-class decisions that previously lived inline
' in Form_FormNCAuditoriaDocumentos.cls event handlers:
...
' Slice:    6a of 25 (Documentos group -- first batch: NCAuditoria side; companion is
'           modFrmNCProyectoDocumentosHelper.bas on NCProyecto side; remaining 3
'           forms in slice 6b).
' =============================================================================

' ----- Message contracts (byte-for-byte match with Test_modFrmNCAuditoriaDocumentosHelper.bas) -----
...
'   blocks. Mirror of modFrmNCProyectoDocumentosHelper_RenderError.
' -----------------------------------------------------------------------------
Public Function modFrmNCAuditoriaDocumentosHelper_RenderError( _
    ...
    If Len(Trim$(p_ErrorText)) = 0 Then
        modFrmNCAuditoriaDocumentosHelper_RenderError = PROMPT_ATOM
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ...
        Else
            style = vbCritical
        End If
        modFrmNCAuditoriaDocumentosHelper_RenderError = MsgBox(p_ErrorText, style, MSG_ERROR_TITLE)
        Exit Function
    End If

    ' Test simulating user response.
    modFrmNCAuditoriaDocumentosHelper_RenderError = p_PromptResult
    Exit Function

EH:
    p_Error = "modFrmNCAuditoriaDocumentosHelper_RenderError: " & Err.Description
    modFrmNCAuditoriaDocumentosHelper_RenderError = vbOK
End Function
...
'   - DocumentoAuditoriaOperaciones.Eliminar checks (1) the file is not open,
'     (2) AR consistency (only one doc + cerrada), (3) cache invalidation,
'     (4) log write. The helper propagates those errors verbatim.
' -----------------------------------------------------------------------------
Public Function modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento( _
    ...
    If p_Doc Is Nothing Then
        modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento = BuildFail(logs, "ConfirmarEliminacionDocumento: p_Doc is Nothing")
        Exit Function
    End If

    ' Atom mode: return prompt text contract. No real modal.
    If p_PromptResult = PROMPT_ATOM Then
        modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento = BuildOk(logs, MSG_ELIMINACION_PROMPT)
        Exit Function
    End If
    ...
    If respuesta <> VB_YES Then
        modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento = BuildOk(logs, "user_rejected")
        Exit Function
    End If
    ...
    If opErr <> "" Then
        p_Error = opErr
        modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento = BuildFail(logs, p_Error)
        Exit Function
    End If

    modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento = BuildOk(logs, "deleted")
    Exit Function

EH:
    p_Error = "modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento: " & Err.Description
    modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento = BuildFail(logs, p_Error)
End Function

' -----------------------------------------------------------------------------
' ValidarNombreDocumento: encapsulate the 4 validation branches in
'   Form_FormNCAuditoriaDocumentos.ComandoCambiarNombre_Click (lines 347-377).
'   Mirror of modFrmNCProyectoDocumentosHelper_ValidarNombreDocumento.
' -----------------------------------------------------------------------------
Public Function modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento( _
    ...
    If p_DocActual Is Nothing Then
        modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento = BuildFail(logs, "ValidarNombreDocumento: p_DocActual is Nothing")
        Exit Function
    End If
    ...
    If Len(Trim$(p_NombreNuevo)) = 0 Then
        ...
        modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento = BuildFail(logs, p_Error)
        Exit Function
    End If
    ...
    ' (4) OK.
    modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento = BuildOk(logs, "ok")
    Exit Function

EH:
    p_Error = "modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento: " & Err.Description
    modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento = BuildFail(logs, p_Error)
End Function