...
' Pure boolean version of the audit form's TodosParametrosBusquedaVacios.
' Checks the 8 audit-specific search controls (Auditoria, Tipo, Descripcion,
' RESPONSABLEIMPLANTACION, Estado, PalabraClave, ComboRequiereControlEficacia,
' ComboControlEficaciaRelleno). Returns True when all are empty so the caller
' can short-circuit the per-row filter loop. Mirrors the project-side helper
' but with audit-specific params. All params are String to match the form's
' Nz(Me.X, "") semantics — the form treats even Auditoria as a stringy
' control value, not a Long ID, for the empty check.
' SDD: cache-form-filter-coverage — decouple audit form's empty-search check
' out of Form_FormNCAuditoriaGestion.cls so it can be tested independently
' and shared if more callers need it.
Public Function TodosParametrosBusquedaVacios( _
            ByVal p_Auditoria As String, _
            ...
            ByVal p_ControlEficaciaRelleno As String) As Boolean

    TodosParametrosBusquedaVacios = (p_Auditoria = "" And p_Tipo = "" And _
                    p_Descripcion = "" And p_ResponsableImplantacion = "" And _
                    p_Estado = "" And p_PalabraClave = "" And _
                    p_RequiereControlEficacia = "" And p_ControlEficaciaRelleno = "")
End Function
