...
Private Function GetNCsProyectoGestionFallback( _
    ...
    For Each id In sourceCol
        ...
        If ShouldIncludeFallbackNC( _
            ...
            If vm.CargarDesdeNCProyecto(nc, vmErr) Then
                resultCol.Add vm, CStr(vm.IDNoConformidad)
            End If
            Set vm = Nothing
        End If
        Set nc = Nothing
...
Private Function GetFallbackSource( _
    ...
    p_Error = ""
    ' Match Form_FormNCProyectoGestion.cls source selection exactly (zero
    ' regression rule + cache-form-filter-coverage W4b):
    '   Branch A: enum=0 (incluye EstadoNC vacío) → getNCsProyectoAbiertas
    '   Branch B: enum>0 + Google → getNCsProyectoPorPalabraClave
    '   Branch C: enum>0 + EstadoValor="Abiertas" → getNCsProyectosTotalesParaAbiertas
    '   Branch D: enum>0 + EstadoValor otro → getNCsProyectosTotales
    ...
    ElseIf p_EstadoValor = "Abiertas" Then
        Set GetFallbackSource = constructor.getNCsProyectosTotalesParaAbiertas(p_Error:=p_Error)
    Else
...
' Pure boolean version of the form's TodosParametrosBusquedaVacios. Includes
' EstadoValor and Google on top of the per-NC filter set (HasAnyFilter) so the
' caller can short-circuit the entire filter + source-selection cycle when the
' search panel is empty. Equivalent to NOT HasAnyFilter(...) AND p_Google = "".
' SDD: cache-form-filter-coverage — decouple form-level empty-search check
' out of Form_FormNCProyectoGestion.cls so it can be tested independently.
Public Function TodosParametrosBusquedaVacios( _
            ByVal p_Codigo As String, _
            ByVal p_IDExpediente As Long, _
            ByVal p_Juridica As String, _
            ByVal p_IDTipo As Long, _
            ...
            ByVal p_ResponsableTelefonica As String, _
            ByVal p_ResponsableCalidad As String, _
            ByVal p_Google As String) As Boolean

    TodosParametrosBusquedaVacios = (p_Codigo = "" And p_IDExpediente = 0 And _
                    p_Juridica = "" And p_IDTipo = 0 And _
                    p_EstadoValor = "" And p_Descripcion = "" And p_Notas = "" And _
                    p_RequiereControlEficacia = "" And p_ControlEficaciaRelleno = "" And _
                    p_RegistrosCerrados = "" And _
                    p_ResponsableTelefonica = "" And p_ResponsableCalidad = "" And _
                    p_Google = "")
End Function

' Pure boolean version of the form's CasiTodosParametrosBusquedaVacios. Same set
' as TodosParametrosBusquedaVacios minus EstadoNC and Google. The form's original
' semantic checked whether all per-NC filter fields were empty; EstadoNC and
' Google affect source selection (Branch A/B/C/D) and were deliberately excluded
' from this lighter check.
Public Function CasiTodosParametrosBusquedaVacios( _
            ByVal p_Codigo As String, _
            ByVal p_IDExpediente As Long, _
            ByVal p_Juridica As String, _
            ByVal p_IDTipo As Long, _
            ...
            ByVal p_RegistrosCerrados As String, _
            ByVal p_ResponsableTelefonica As String, _
            ByVal p_ResponsableCalidad As String) As Boolean

    CasiTodosParametrosBusquedaVacios = (p_Codigo = "" And p_IDExpediente = 0 And _
                    p_Juridica = "" And p_IDTipo = 0 And _
                    p_Descripcion = "" And p_Notas = "" And _
                    p_RequiereControlEficacia = "" And p_ControlEficaciaRelleno = "" And _
                    p_RegistrosCerrados = "" And _
                    p_ResponsableTelefonica = "" And p_ResponsableCalidad = "")
End Function
