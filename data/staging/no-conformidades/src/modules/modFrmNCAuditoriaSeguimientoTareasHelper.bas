Attribute VB_Name = "modFrmNCAuditoriaSeguimientoTareasHelper"
Option Compare Database
Option Explicit

' =============================================================================
' Form helper for Form_FormNCAuditoriaSeguimientoTareas (slice 7 of form-thin-helper-refactor).
'
' This helper owns the 1 message-class decision that previously lived inline
' in Form_FormNCAuditoriaSeguimientoTareas.cls event handlers:
...
' Slice:    7 of 25 (SeguimientoTareas pair -- NCAuditoria side; companion is
'           modFrmNCProyectoSeguimientoTareasHelper.bas on NCProyecto side).
' =============================================================================

' ----- Message contracts (byte-for-byte match with Test_modFrmNCAuditoriaSeguimientoTareasHelper.bas) -----
...
'   blocks (mirrors modFrmNCProyectoSeguimientoTareasHelper_RenderError shape).
'
...
'         End If
'         pregunta = modFrmNCAuditoriaSeguimientoTareasHelper_RenderError(m_Error, Err.Number, , m_HelperErr)
'
...
' -----------------------------------------------------------------------------
Public Function modFrmNCAuditoriaSeguimientoTareasHelper_RenderError( _
    ...
    If Len(Trim$(p_ErrorText)) = 0 Then
        modFrmNCAuditoriaSeguimientoTareasHelper_RenderError = PROMPT_ATOM
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ...
        Else
            style = vbCritical
        End If
        modFrmNCAuditoriaSeguimientoTareasHelper_RenderError = MsgBox(p_ErrorText, style, MSG_ERROR_TITLE)
        Exit Function
    End If

    ' Test simulating user response.
    modFrmNCAuditoriaSeguimientoTareasHelper_RenderError = p_PromptResult
    Exit Function

EH:
    p_Error = "modFrmNCAuditoriaSeguimientoTareasHelper_RenderError: " & Err.Description
    modFrmNCAuditoriaSeguimientoTareasHelper_RenderError = vbOK
End Function
