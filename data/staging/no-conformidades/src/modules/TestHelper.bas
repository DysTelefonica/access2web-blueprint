...
' GetTestDB
'
' Returns the DAO.Database connection for the current test execution context.
' In m_TestingMode, the project-wide getdb() routes to the sandbox backend
' (m_BackendSandboxURL from Variables Globales.bas). In production mode it
' returns the live backend. The caller is responsible for BeginTestSession()
' having activated m_TestingMode before calling.
'
' Used by slice 4 (Form_FormAuditoriasGestionHelper tests) and any other test
' that needs the sandbox db handle without going through getdb() directly.
' -----------------------------------------------------------------------------
Public Function GetTestDB() As DAO.Database
    On Error GoTo EH
    Dim m_Error As String
    Set GetTestDB = getdb(m_Error)
    If m_Error <> "" Then Err.Raise 1000, "GetTestDB", m_Error
    Exit Function
EH:
    Err.Raise Err.Number, "GetTestDB", Err.Description
End Function
