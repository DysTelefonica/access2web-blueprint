Attribute VB_Name = "modFrmNCAuditoriaARHelper"
Option Compare Database
Option Explicit

' =============================================================================
' Form helper for Form_FormNCAuditoriaAR (slice 5 of form-thin-helper-refactor).
'
' This helper owns the 3 message-class decisions that previously lived inline
' in Form_FormNCAuditoriaAR.cls event handlers:
'   1. _RenderError             -- boilerplate error handler (~14 of 16 MsgBox)
'   2. _ConfirmarBorradoNC      -- MsgBox("¿Desea borrar la AR?") + parent-NC Borrar flow
...
' Slice:    5 of 27 (AR pair: NCAuditoriaAR; mirror of 5a NCProyecto
'           side; companion is modFrmNCProyectoARHelper.bas).
' =============================================================================

' ----- Message contracts (byte-for-byte match with Test_modFrmNCAuditoriaARHelper.bas) -----
...
' -----------------------------------------------------------------------------
Public Function modFrmNCAuditoriaARHelper_RenderError( _
    ...
    If Len(Trim$(p_ErrorText)) = 0 Then
        modFrmNCAuditoriaARHelper_RenderError = PROMPT_ATOM
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ...
        Else
            style = vbCritical
        End If
        modFrmNCAuditoriaARHelper_RenderError = MsgBox(p_ErrorText, style, MSG_ERROR_TITLE)
        Exit Function
    End If

    ' Test simulating user response.
    modFrmNCAuditoriaARHelper_RenderError = p_PromptResult
    Exit Function

EH:
    p_Error = "modFrmNCAuditoriaARHelper_RenderError: " & Err.Description
    modFrmNCAuditoriaARHelper_RenderError = vbOK
End Function
...
'   delete lives in FormNCAuditoriaAcciones, not in the single-AR edit form).
...
' -----------------------------------------------------------------------------
Public Function modFrmNCAuditoriaARHelper_ConfirmarBorradoNC( _
    ...
    If p_NC Is Nothing Then
        modFrmNCAuditoriaARHelper_ConfirmarBorradoNC = BuildFail(logs, "ConfirmarBorradoNC: p_NC is Nothing")
        Exit Function
    End If

    ' Atom mode: return prompt text contract. No real modal.
    If p_PromptResult = PROMPT_ATOM Then
        modFrmNCAuditoriaARHelper_ConfirmarBorradoNC = BuildOk(logs, MSG_BORRADO_PROMPT)
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ' Production: show real MsgBox.
        respuesta = MsgBox( _
            "¿Desea borrar la Acción Realizada de auditoría seleccionada?", _
            vbExclamation + vbYesNo + vbDefaultButton2, MSG_BORRADO_TITLE)
        If respuesta <> VB_YES Then
            modFrmNCAuditoriaARHelper_ConfirmarBorradoNC = BuildOk(logs, "user_rejected")
            Exit Function
        End If
    ...
    If Len(Trim$(motivoFinal)) = 0 Then
        ...
        modFrmNCAuditoriaARHelper_ConfirmarBorradoNC = BuildFail(logs, p_Error)
        Exit Function
    End If
    ...
    modFrmNCAuditoriaARHelper_ConfirmarBorradoNC = BuildOk(logs, "deleted")
    Exit Function

EH:
    p_Error = "modFrmNCAuditoriaARHelper_ConfirmarBorradoNC: " & Err.Description
    modFrmNCAuditoriaARHelper_ConfirmarBorradoNC = BuildFail(logs, p_Error)
End Function
...
'   habilitacion flow lives in FormNCAuditoriaAcciones, not in the single-AR
...
' -----------------------------------------------------------------------------
Public Function modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC( _
                                            ByRef p_NC As NCAuditoria, _
                                            Optional ByRef db As DAO.Database = Nothing, _
                                            Optional ByRef p_PromptResult As Long = 0, _
                                            Optional ByRef p_Error As String = "" _
    ...
    ' Adversarial: p_NC is Nothing.
    If p_NC Is Nothing Then
        modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC = BuildFail(logs, "ConfirmarHabilitacionNC: p_NC is Nothing")
        Exit Function
    End If

    ' Atom mode: return prompt text contract. No real modal.
    If p_PromptResult = PROMPT_ATOM Then
        modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC = BuildOk(logs, MSG_HABILITACION_PROMPT)
        Exit Function
    End If
    ...
    If respuesta <> VB_YES Then
        modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC = BuildOk(logs, "user_rejected")
        Exit Function
    End If
    ...
    If p_NC.Borrado = False Then
        ...
        modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC = BuildFail(logs, p_Error)
        Exit Function
    End If
    ...
    modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC = BuildOk(logs, "habilitada")
    Exit Function

EH:
    p_Error = "modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC: " & Err.Description
    modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC = BuildFail(logs, p_Error)
End Function