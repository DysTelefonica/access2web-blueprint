Attribute VB_Name = "modFrmNCProyectoSeguimientoNCHelper"
Option Compare Database
Option Explicit

' =============================================================================
' Form helper for Form_FormNCProyectoSeguimientoNC (slice 8 of form-thin-helper-refactor).
'
' This helper owns the 1 message-class decision that previously lived inline
' in Form_FormNCProyectoSeguimientoNC.cls event handlers:
...
' Slice:    8 of 25 (SeguimientoNC pair -- NCProyecto side; companion is
'           modFrmNCAuditoriaSeguimientoNCHelper.bas on NCAuditoria side).
' =============================================================================

' ----- Message contracts (byte-for-byte match with Test_modFrmNCProyectoSeguimientoNCHelper.bas) -----
...
'         End If
'         pregunta = modFrmNCProyectoSeguimientoNCHelper_RenderError(m_Error, Err.Number, , m_HelperErr)
'
...
' -----------------------------------------------------------------------------
Public Function modFrmNCProyectoSeguimientoNCHelper_RenderError( _
    ...
    If Len(Trim$(p_ErrorText)) = 0 Then
        modFrmNCProyectoSeguimientoNCHelper_RenderError = PROMPT_ATOM
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ...
        Else
            style = vbCritical
        End If
        modFrmNCProyectoSeguimientoNCHelper_RenderError = MsgBox(p_ErrorText, style, MSG_ERROR_TITLE)
        Exit Function
    End If

    ' Test simulating user response.
    modFrmNCProyectoSeguimientoNCHelper_RenderError = p_PromptResult
    Exit Function

EH:
    p_Error = "modFrmNCProyectoSeguimientoNCHelper_RenderError: " & Err.Description
    modFrmNCProyectoSeguimientoNCHelper_RenderError = vbOK
End Function