Attribute VB_Name = "modFrmNCProyectoSeguimientoTareasHelper"
Option Compare Database
Option Explicit

' =============================================================================
' Form helper for Form_FormNCProyectoSeguimientoTareas (slice 7 of form-thin-helper-refactor).
'
' This helper owns the 1 message-class decision that previously lived inline
' in Form_FormNCProyectoSeguimientoTareas.cls event handlers:
...
' Slice:    7 of 25 (SeguimientoTareas pair -- NCProyecto side; companion is
'           modFrmNCAuditoriaSeguimientoTareasHelper.bas on NCAuditoria side).
' =============================================================================

' ----- Message contracts (byte-for-byte match with Test_modFrmNCProyectoSeguimientoTareasHelper.bas) -----
...
'   blocks (mirrors modFrmNCAuditoriaDocumentosHelper_RenderError shape).
'
...
'         End If
'         pregunta = modFrmNCProyectoSeguimientoTareasHelper_RenderError(m_Error, Err.Number, , m_HelperErr)
'
...
' -----------------------------------------------------------------------------
Public Function modFrmNCProyectoSeguimientoTareasHelper_RenderError( _
    ...
    If Len(Trim$(p_ErrorText)) = 0 Then
        modFrmNCProyectoSeguimientoTareasHelper_RenderError = PROMPT_ATOM
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ...
        Else
            style = vbCritical
        End If
        modFrmNCProyectoSeguimientoTareasHelper_RenderError = MsgBox(p_ErrorText, style, MSG_ERROR_TITLE)
        Exit Function
    End If

    ' Test simulating user response.
    modFrmNCProyectoSeguimientoTareasHelper_RenderError = p_PromptResult
    Exit Function

EH:
    p_Error = "modFrmNCProyectoSeguimientoTareasHelper_RenderError: " & Err.Description
    modFrmNCProyectoSeguimientoTareasHelper_RenderError = vbOK
End Function
