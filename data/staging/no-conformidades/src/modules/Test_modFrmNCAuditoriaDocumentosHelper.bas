Attribute VB_Name = "Test_modFrmNCAuditoriaDocumentosHelper"
Option Compare Database
Option Explicit

' =============================================================================
' Atoms for modFrmNCAuditoriaDocumentosHelper (slice 6a of form-thin-helper-refactor).
'
...
'           inline in Form_FormNCAuditoriaDocumentos.cls:
...
' Slice:    6a of 25 (Documentos group -- first batch: NCAuditoria side).
...
'   - TbDocumentosAuditorias columns: same as TbNCDocumentos (IDDocumento,
'     IDNoConformidad, Documento); table name verified via
'     DocumentoAuditoria.cls line 158: constructor.getID("TbDocumentosAuditorias", ...)
'   - TbNoConformidadesAuditoria PK column is lowercase `id` (NOT
'     IDNoConformidad) -- verified via working INSERT in
'     Test_modFrmNCAuditoriaAccionesHelper.bas lines 76-88.
' =============================================================================

' ----- Message contract (byte-for-byte match with production literals) -----
Private Const MSG_ELIMINACION_PROMPT As String = "MSG-NCAUDITORIA-DOCUMENTOS-ELIMINACION: ¿Desea borrar el documento seleccionado?"
...
' ----- Fixture IDs (>=900662000 reserved for this module, access-vba-tdd §1.7) -----
Private Const FIX_ID_NC As Long = 900662001
Private Const FIX_ID_DOC As Long = 900662002
...
' NOTE: PK column is lowercase `id` (NOT IDNoConformidad) for the
' TbNoConformidadesAuditoria table -- verified via working INSERT in
' Test_modFrmNCAuditoriaAccionesHelper.bas lines 76-88.
Private Function EnsureNCAuditoriaFixtureForSlice6a(ByVal p_Db As DAO.Database, _
                                                     ByVal p_IDNC As Long, _
                                                     ByRef p_Error As String) As Boolean
    On Error GoTo EH
    EnsureNCAuditoriaFixtureForSlice6a = False
    p_Error = ""
    ...
    p_Db.Execute "INSERT INTO TbNoConformidadesAuditoria (id, DESCRIPCION, ESTADO, FECHAAPERTURA, Tipo) " & _
                 ...
                 "'NC fixture slice 6a auditoria documentos', " & _
                 "'REGISTRADA', #2026-06-26#, " & _
                 "'Auditoria')", dbFailOnError
    EnsureNCAuditoriaFixtureForSlice6a = True
    Exit Function
EH:
    p_Error = "EnsureNCAuditoriaFixtureForSlice6a: " & Err.Description
End Function
...
Private Function BuildDocumentoAuditoriaForSlice6a(ByVal p_IDDoc As Long, _
                                                    ByVal p_IDNC As Long, _
    ...
    Set BuildDocumentoAuditoriaForSlice6a = doc
End Function
...
' =============================================================================
Public Function Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Happy_NoModal_Atomic() As String
    ...
    returnCode = modFrmNCAuditoriaDocumentosHelper_RenderError( _
        "Al btnExaminar_Click se ha producido el error n: 9" & vbNewLine & "Detalle: subscript out of range", _
        9, -1, helperErrMsg)

    If helperErrMsg <> "" Then
        Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Happy_NoModal_Atomic = _
...
EH:
    Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Happy_NoModal_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Happy_NoModal_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 2 - Sad: _RenderError with Err.Number=1000 (warning path).
' =============================================================================
Public Function Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Sad_WarningPath_Atomic() As String
    Dim logs As Collection
    Dim returnCode As Long
    Dim helperErrMsg As String
    ...
    returnCode = modFrmNCAuditoriaDocumentosHelper_RenderError( _
        "Debe poner un título al documento.", 1000, -1, helperErrMsg)

    If helperErrMsg <> "" Then
        Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Sad_WarningPath_Atomic = _
            TestHelper.BuildJsonFail("helperErrMsg should be empty (got='" & helperErrMsg & "')", logs)
        Exit Function
    End If
    TestHelper.AddLog logs, "Assert OK: helperErrMsg empty"
...
EH:
    Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Sad_WarningPath_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Sad_WarningPath_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 3 - Edge: _RenderError with empty m_ErrorText returns -1, no modal.
' =============================================================================
Public Function Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Edge_EmptyText_Atomic() As String
    Dim logs As Collection
    Dim returnCode As Long
    Dim helperErrMsg As String
    ...
    If helperErrMsg <> "" Then
        Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Edge_EmptyText_Atomic = _
            TestHelper.BuildJsonFail("helperErrMsg should be empty (got='" & helperErrMsg & "')", logs)
        Exit Function
    End If
    TestHelper.AddLog logs, "Assert OK: helperErrMsg empty"
...
EH:
    Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Edge_EmptyText_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Edge_EmptyText_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 4 - Adversarial: _RenderError with very long error text.
' =============================================================================
Public Function Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Adversarial_VeryLongText_Atomic() As String
    Dim logs As Collection
    Dim returnCode As Long
    Dim helperErrMsg As String
    ...
    If helperErrMsg <> "" Then
        Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Adversarial_VeryLongText_Atomic = _
            TestHelper.BuildJsonFail("helperErrMsg should be empty (got='" & helperErrMsg & "')", logs)
        Exit Function
    End If
    TestHelper.AddLog logs, "Assert OK: helperErrMsg empty"
...
EH:
    Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Adversarial_VeryLongText_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaDocumentosHelper_RenderError_Adversarial_VeryLongText_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 5 - Happy: _ConfirmarEliminacionDocumento atom mode returns prompt text.
' =============================================================================
Public Function Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Happy_ReturnsPromptText_Atomic() As String
    Dim logs As Collection
    ...
    Set doc = BuildDocumentoAuditoriaForSlice6a(FIX_ID_DOC, FIX_ID_NC, "fixture_doc_eliminar.pdf")

    resultJson = modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento( _
        doc, Nothing, -1, helperErrMsg)

    If helperErrMsg <> "" Then
        Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Happy_ReturnsPromptText_Atomic = _
            TestHelper.BuildJsonFail("helperErrMsg should be empty (got='" & helperErrMsg & "')", logs)
        Exit Function
    End If
    TestHelper.AddLog logs, "Assert OK: helperErrMsg empty"
...
EH:
    Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Happy_ReturnsPromptText_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Happy_ReturnsPromptText_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 6 - Sad: _ConfirmarEliminacionDocumento with vbNo returns user_rejected.
' =============================================================================
Public Function Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Sad_UserRejected_Atomic() As String
    Dim logs As Collection
    Dim resultJson As String
    Dim helperErrMsg As String
    Dim doc As DocumentoAuditoria
    ...
    If helperErrMsg <> "" Then
        Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Sad_UserRejected_Atomic = _
            TestHelper.BuildJsonFail("helperErrMsg should be empty (got='" & helperErrMsg & "')", logs)
        Exit Function
    End If
    TestHelper.AddLog logs, "Assert OK: helperErrMsg empty"
...
EH:
    Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Sad_UserRejected_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Sad_UserRejected_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 7 - Edge: _ConfirmarEliminacionDocumento with vbYes passes the gate.
' =============================================================================
Public Function Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Edge_UserConfirms_Atomic() As String
    Dim logs As Collection
    Dim resultJson As String
    Dim helperErrMsg As String
    ...
    If Not TestHelper.BeginTestSession(logs) Then
        Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Edge_UserConfirms_Atomic = _
            TestHelper.BuildJsonFail("BeginTestSession failed", logs)
        GoTo Cleanup
    End If
    Set db = getdb()
    If Not EnsureNCAuditoriaFixtureForSlice6a(db, FIX_ID_NC, helperErrMsg) Then
        Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Edge_UserConfirms_Atomic = _
            TestHelper.BuildJsonFail("EnsureNCAuditoriaFixtureForSlice6a: " & helperErrMsg, logs)
        GoTo Cleanup
    End If
...
EH:
    Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Edge_UserConfirms_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Edge_UserConfirms_Atomic: " & Err.Description, logs)
...
' =============================================================================
Public Function Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Adversarial_NothingDoc_Atomic() As String
    Dim logs As Collection
    Dim resultJson As String
    Dim helperErrMsg As String
    ...
    If InStr(1, resultJson, "Nothing", vbTextCompare) = 0 Then
        Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Adversarial_NothingDoc_Atomic = _
...
EH:
    Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Adversarial_NothingDoc_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaDocumentosHelper_ConfirmarEliminacionDocumento_Adversarial_NothingDoc_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 9 - Happy: _ValidarNombreDocumento with unique new name returns ok=true.
' =============================================================================
Public Function Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Happy_UniqueName_Atomic() As String
    Dim logs As Collection
    Dim resultJson As String
    Dim helperErrMsg As String
    ...
    Set docActual = BuildDocumentoAuditoriaForSlice6a(FIX_ID_DOC, FIX_ID_NC, "original_doc.pdf")
    Set docOtro = BuildDocumentoAuditoriaForSlice6a(FIX_ID_DOC + 100, FIX_ID_NC, "otro_doc.pdf")
    ...
    resultJson = modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento( _
        "nuevo_nombre_unico.pdf", docActual, colDocs, -1, helperErrMsg)

    If helperErrMsg <> "" Then
        Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Happy_UniqueName_Atomic = _
            TestHelper.BuildJsonFail("helperErrMsg should be empty (got='" & helperErrMsg & "')", logs)
        Exit Function
    End If
    TestHelper.AddLog logs, "Assert OK: helperErrMsg empty"
...
EH:
    Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Happy_UniqueName_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Happy_UniqueName_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 10 - Sad: _ValidarNombreDocumento with empty new name returns MSG_NOMBRE_VACIO.
' =============================================================================
Public Function Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Sad_EmptyName_Atomic() As String
    Dim logs As Collection
    Dim resultJson As String
    Dim helperErrMsg As String
    Dim docActual As DocumentoAuditoria
    ...
    If InStr(1, resultJson, MSG_NOMBRE_VACIO, vbTextCompare) = 0 Then
        Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Sad_EmptyName_Atomic = _
...
EH:
    Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Sad_EmptyName_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Sad_EmptyName_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 11 - Edge: _ValidarNombreDocumento with same-as-current returns MSG_NOMBRE_IGUAL.
' =============================================================================
Public Function Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Edge_SameName_Atomic() As String
    Dim logs As Collection
    Dim resultJson As String
    Dim helperErrMsg As String
    Dim docActual As DocumentoAuditoria
    ...
    If InStr(1, resultJson, MSG_NOMBRE_IGUAL, vbTextCompare) = 0 Then
        Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Edge_SameName_Atomic = _
...
EH:
    Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Edge_SameName_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Edge_SameName_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 12 - Adversarial: _ValidarNombreDocumento with duplicate in collection.
' =============================================================================
Public Function Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Adversarial_DuplicateName_Atomic() As String
    Dim logs As Collection
    Dim resultJson As String
    Dim helperErrMsg As String
    Dim docActual As DocumentoAuditoria
    ...
    Set docActual = BuildDocumentoAuditoriaForSlice6a(FIX_ID_DOC, FIX_ID_NC, "original_doc.pdf")
    Set docOtro = BuildDocumentoAuditoriaForSlice6a(FIX_ID_DOC + 100, FIX_ID_NC, "nombre_que_ya_existe.pdf")
    ...
    If InStr(1, resultJson, MSG_NOMBRE_DUPLICADO, vbTextCompare) = 0 Then
        Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Adversarial_DuplicateName_Atomic = _
...
EH:
    Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Adversarial_DuplicateName_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaDocumentosHelper_ValidarNombreDocumento_Adversarial_DuplicateName_Atomic: " & Err.Description, logs)
End Function