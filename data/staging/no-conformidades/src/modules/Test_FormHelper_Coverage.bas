...
' ============================================
' TEST No-user resilience (integration test — helper has no db injection)
' ============================================
Public Function Test_FormHelper_Listing_NoUserResilient_Atomic() As String
    Dim logs As Collection
    Dim db As DAO.Database
    Dim sessionErr As String
    Dim assertError As String
    ...
    ' Default to fail so HandleError never leaves result=Empty (silent pass)
    Set logs = TestHelper.NewLogs
    Test_FormHelper_Listing_NoUserResilient_Atomic = TestHelper.BuildJsonFail("test did not complete", logs)

    On Error GoTo HandleError

    ' === SETUP ===
    If Not TestHelper.BeginTestSession(logs, sessionErr) Then
        Test_FormHelper_Listing_NoUserResilient_Atomic = TestHelper.BuildJsonFail("TESTS BLOCKED: " & sessionErr, logs)
        Exit Function
    End If
    ...
    ' Clean state + seed a known source NC. The fallback path runs because the
    ' cache is empty for the seeded NC (CacheHabilitada=True with 0 valid cache
    ' rows forces fallback to legacy source selection). This exercises the
    ' no-user code path: SafeFallbackUser(Nothing) inside LogFallback must
    ' return a safe placeholder, not raise.
    Call CleanupFallbackFixture(db, logs)
    Call WriteCacheHabilitadaToDb(db, True)
    Call SeedFallbackNCFixture(db, TEST_ID_NC_FALLBACK_EMPTY, "FNHC-NO-USER", "SC-1.X no-user resilience source row", logs)
    Set m_ObjUsuarioConectado = Nothing
    TestHelper.AddLog logs, "Arrange SC-1.X: cache enabled, 1 deterministic source NC seeded with open AC+AR, m_ObjUsuarioConectado=Nothing"

    countBefore = CountRowsWhereLocal(db, "TbNoConformidades", "IDNoConformidad=" & TEST_ID_NC_FALLBACK_EMPTY)
    If countBefore <> 1 Then
        Test_FormHelper_Listing_NoUserResilient_Atomic = TestHelper.BuildJsonFail("fixture seed failed: expected 1 NC row, got " & CStr(countBefore), logs)
        GoTo Cleanup
    End If
    ...
    ' === ASSERT ===
    If helperErr <> "" Then
        Test_FormHelper_Listing_NoUserResilient_Atomic = TestHelper.BuildJsonFail("helper raised p_Error with no user: " & helperErr, logs)
        GoTo Cleanup
    End If
    If Not TestHelper.AssertTrue(Not col Is Nothing, "Helper must return non-Nothing Collection when m_ObjUsuarioConectado=Nothing", logs, assertError) Then
        Test_FormHelper_Listing_NoUserResilient_Atomic = TestHelper.BuildJsonFail(assertError, logs)
        GoTo Cleanup
    End If
    If Not TestHelper.AssertTrue(CollectionContainsId(col, TEST_ID_NC_FALLBACK_EMPTY), _
            "Collection must include the seeded source NC (id=" & CStr(TEST_ID_NC_FALLBACK_EMPTY) & ") regardless of m_ObjUsuarioConectado=Nothing", _
            logs, assertError) Then
        Test_FormHelper_Listing_NoUserResilient_Atomic = TestHelper.BuildJsonFail(assertError, logs)
        GoTo Cleanup
    End If
    TestHelper.AddLog logs, "Assert: helper returned Collection count=" & CStr(col.count) & " with m_ObjUsuarioConectado=Nothing; seeded NC present"

    Test_FormHelper_Listing_NoUserResilient_Atomic = TestHelper.BuildJsonOk(logs, "no_user_resilient_ok")
    GoTo Cleanup
...
HandleError:
    Test_FormHelper_Listing_NoUserResilient_Atomic = TestHelper.BuildJsonFail("unexpected error: " & Err.Description & " (source: " & Err.Source & ")", logs)
    Resume Cleanup
End Function

' ============================================
' TEST TodosParametrosBusquedaVacios (helper pure function)
' ============================================
Public Function Test_FormHelper_TodosParametrosBusquedaVacios_AllEmpty_Atomic() As String
    Dim logs As Collection
    Dim assertError As String
    Dim result As Boolean

    Set logs = TestHelper.NewLogs
    Test_FormHelper_TodosParametrosBusquedaVacios_AllEmpty_Atomic = TestHelper.BuildJsonFail("test did not complete", logs)

    On Error GoTo HandleError

    result = NCProyectoGestionListadoHelper.TodosParametrosBusquedaVacios( _
                    "", 0, "", 0, "", "", "", "", "", "", "", "", "")
    If Not TestHelper.AssertTrue(result, "All-empty search panel must return True", logs, assertError) Then GoTo Fail
    TestHelper.AddLog logs, "Assert: all-empty inputs -> True"

    Test_FormHelper_TodosParametrosBusquedaVacios_AllEmpty_Atomic = TestHelper.BuildJsonOk(logs, "all_empty_true")
    GoTo Cleanup

Fail:
    Test_FormHelper_TodosParametrosBusquedaVacios_AllEmpty_Atomic = TestHelper.BuildJsonFail(assertError, logs)
    GoTo Cleanup
    Exit Function

HandleError:
    Test_FormHelper_TodosParametrosBusquedaVacios_AllEmpty_Atomic = TestHelper.BuildJsonFail("unexpected error: " & Err.Description, logs)
    Resume Cleanup
...
Private Sub SeedFallbackNCFixture( _
    ...
    ' Deterministic AC/AR ids for this fixture, out of the 900xxx test range.
    Dim idAC As Long
    Dim idAR As Long
    idAC = 920000 + (p_IDNC Mod 1000)
    idAR = 930000 + (p_IDNC Mod 1000)

    p_Db.Execute "DELETE FROM TbNCAccionesRealizadas WHERE IDAccionRealizada=" & CStr(idAR), dbFailOnError
    p_Db.Execute "DELETE FROM TbNCAccionCorrectivas WHERE IDAccionCorrectiva=" & CStr(idAC), dbFailOnError
    ...
    ' Seed open AC+AR so getNCsProyectoAbiertas (Branch A) JOIN returns the NC.
    p_Db.Execute "INSERT INTO TbNCAccionCorrectivas (IDAccionCorrectiva, IDNoConformidad, NAccion, AccionCorrectiva, ESTADO) VALUES (" & _
                 CStr(idAC) & ", " & CStr(p_IDNC) & ", 1, " & SqlLiteralLocal("AC fallback " & p_Codigo) & ", " & SqlLiteralLocal("Pendiente") & ")", dbFailOnError
    p_Db.Execute "INSERT INTO TbNCAccionesRealizadas (IDAccionRealizada, IDAccionCorrectiva, NAccion, AccionRealizada, Estado, FechaFinReal) VALUES (" & _
                 CStr(idAR) & ", " & CStr(idAC) & ", 1, " & SqlLiteralLocal("AR fallback " & p_Codigo) & ", " & SqlLiteralLocal("Pendiente") & ", Null)", dbFailOnError
    TestHelper.AddLog p_Logs, "Seed: fallback source NC=" & CStr(p_IDNC) & " Codigo=" & p_Codigo & " with open AC=" & CStr(idAC) & " AR=" & CStr(idAR)
End Sub
...
Private Sub CleanupFallbackFixture(ByVal p_Db As DAO.Database, ByRef p_Logs As Collection)
    On Error Resume Next
    ' Compute deterministic AC/AR ids that match the seed helper.
    Dim idACEmpty As Long, idAREmpty As Long
    Dim idACDisabled As Long, idARDisabled As Long
    idACEmpty = 920000 + (TEST_ID_NC_FALLBACK_EMPTY Mod 1000)
    idAREmpty = 930000 + (TEST_ID_NC_FALLBACK_EMPTY Mod 1000)
    idACDisabled = 920000 + (TEST_ID_NC_FALLBACK_DISABLED Mod 1000)
    idARDisabled = 930000 + (TEST_ID_NC_FALLBACK_DISABLED Mod 1000)
    ...
    ' Reverse FK order: AR -> AC -> NC.
    p_Db.Execute "DELETE FROM TbNCAccionesRealizadas WHERE IDAccionRealizada IN (" & CStr(idAREmpty) & "," & CStr(idARDisabled) & ")", dbFailOnError
    p_Db.Execute "DELETE FROM TbNCAccionCorrectivas WHERE IDAccionCorrectiva IN (" & CStr(idACEmpty) & "," & CStr(idACDisabled) & ")", dbFailOnError
