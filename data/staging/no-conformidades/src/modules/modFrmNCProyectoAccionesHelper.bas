Attribute VB_Name = "modFrmNCProyectoAccionesHelper"
Option Compare Database
Option Explicit

' =============================================================================
' Form helper for Form_FormNCProyectoAcciones (slice 3 of form-thin-helper-refactor).
'
' This helper owns the 3 message-class decisions that previously lived inline
' in Form_FormNCProyectoAcciones.cls event handlers:
...
' Slice:    3 of 27 (Acciones pair: NCProyectoAcciones + NCAuditoriaAcciones;
'           mirror of slice 2b Gestion pair with name changes only).
' =============================================================================

' ----- Message contracts (byte-for-byte match with Test_modFrmNCProyectoAccionesHelper.bas) -----
...
' -----------------------------------------------------------------------------
Public Function modFrmNCProyectoAccionesHelper_RenderError( _
    ...
    If Len(Trim$(p_ErrorText)) = 0 Then
        modFrmNCProyectoAccionesHelper_RenderError = PROMPT_ATOM
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ...
        Else
            style = vbCritical
        End If
        modFrmNCProyectoAccionesHelper_RenderError = MsgBox(p_ErrorText, style, MSG_ERROR_TITLE)
        Exit Function
    End If

    ' Test simulating user response.
    modFrmNCProyectoAccionesHelper_RenderError = p_PromptResult
    Exit Function

EH:
    p_Error = "modFrmNCProyectoAccionesHelper_RenderError: " & Err.Description
    modFrmNCProyectoAccionesHelper_RenderError = vbOK
End Function
...
' -----------------------------------------------------------------------------
Public Function modFrmNCProyectoAccionesHelper_ConfirmarBorradoNC( _
    ...
    If p_NC Is Nothing Then
        modFrmNCProyectoAccionesHelper_ConfirmarBorradoNC = BuildFail(logs, "ConfirmarBorradoNC: p_NC is Nothing")
        Exit Function
    End If

    ' Atom mode: return prompt text contract. No real modal.
    If p_PromptResult = PROMPT_ATOM Then
        modFrmNCProyectoAccionesHelper_ConfirmarBorradoNC = BuildOk(logs, MSG_BORRADO_PROMPT)
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ' Production: show real MsgBox.
        respuesta = MsgBox( _
            "¿Desea borrar la Acción Correctiva seleccionada?", _
            vbExclamation + vbYesNo + vbDefaultButton2, MSG_BORRADO_TITLE)
        If respuesta <> VB_YES Then
            modFrmNCProyectoAccionesHelper_ConfirmarBorradoNC = BuildOk(logs, "user_rejected")
            Exit Function
        End If
    ...
    If Len(Trim$(motivoFinal)) = 0 Then
        ...
        modFrmNCProyectoAccionesHelper_ConfirmarBorradoNC = BuildFail(logs, p_Error)
        Exit Function
    End If
    ...
    modFrmNCProyectoAccionesHelper_ConfirmarBorradoNC = BuildOk(logs, "deleted")
    Exit Function

EH:
    p_Error = "modFrmNCProyectoAccionesHelper_ConfirmarBorradoNC: " & Err.Description
    modFrmNCProyectoAccionesHelper_ConfirmarBorradoNC = BuildFail(logs, p_Error)
End Function
...
'   Returns: JSON string with {ok, value, error, logs}.
' -----------------------------------------------------------------------------
Public Function modFrmNCProyectoAccionesHelper_ConfirmarHabilitacionNC( _
                                            ByRef p_NC As NCProyecto, _
                                            Optional ByRef db As DAO.Database = Nothing, _
                                            Optional ByRef p_PromptResult As Long = 0, _
                                            Optional ByRef p_Error As String = "" _
    ...
    ' Adversarial: p_NC is Nothing.
    If p_NC Is Nothing Then
        modFrmNCProyectoAccionesHelper_ConfirmarHabilitacionNC = BuildFail(logs, "ConfirmarHabilitacionNC: p_NC is Nothing")
        Exit Function
    End If

    ' Atom mode: return prompt text contract. No real modal.
    If p_PromptResult = PROMPT_ATOM Then
        modFrmNCProyectoAccionesHelper_ConfirmarHabilitacionNC = BuildOk(logs, MSG_HABILITACION_PROMPT)
        Exit Function
    End If
    ...
    If respuesta <> VB_YES Then
        modFrmNCProyectoAccionesHelper_ConfirmarHabilitacionNC = BuildOk(logs, "user_rejected")
        Exit Function
    End If
    ...
    If p_NC.Borrado = False Then
        ...
        modFrmNCProyectoAccionesHelper_ConfirmarHabilitacionNC = BuildFail(logs, p_Error)
        Exit Function
    End If
    ...
    modFrmNCProyectoAccionesHelper_ConfirmarHabilitacionNC = BuildOk(logs, "habilitada")
    Exit Function

EH:
    p_Error = "modFrmNCProyectoAccionesHelper_ConfirmarHabilitacionNC: " & Err.Description
    modFrmNCProyectoAccionesHelper_ConfirmarHabilitacionNC = BuildFail(logs, p_Error)
End Function
