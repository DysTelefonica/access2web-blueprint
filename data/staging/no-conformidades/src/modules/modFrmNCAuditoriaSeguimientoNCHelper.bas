Attribute VB_Name = "modFrmNCAuditoriaSeguimientoNCHelper"
Option Compare Database
Option Explicit

' =============================================================================
' Form helper for Form_FormNCAuditoriaSeguimientoNC (slice 8 of form-thin-helper-refactor).
'
' This helper owns the 1 message-class decision that previously lived inline
' in Form_FormNCAuditoriaSeguimientoNC.cls event handlers:
...
' Slice:    8 of 25 (SeguimientoNC pair -- NCAuditoria side; companion is
'           modFrmNCProyectoSeguimientoNCHelper.bas on NCProyecto side).
' =============================================================================

' ----- Message contracts (byte-for-byte match with Test_modFrmNCAuditoriaSeguimientoNCHelper.bas) -----
...
'         End If
'         pregunta = modFrmNCAuditoriaSeguimientoNCHelper_RenderError(m_Error, Err.Number, , m_HelperErr)
'
...
' -----------------------------------------------------------------------------
Public Function modFrmNCAuditoriaSeguimientoNCHelper_RenderError( _
    ...
    If Len(Trim$(p_ErrorText)) = 0 Then
        modFrmNCAuditoriaSeguimientoNCHelper_RenderError = PROMPT_ATOM
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ...
        Else
            style = vbCritical
        End If
        modFrmNCAuditoriaSeguimientoNCHelper_RenderError = MsgBox(p_ErrorText, style, MSG_ERROR_TITLE)
        Exit Function
    End If

    ' Test simulating user response.
    modFrmNCAuditoriaSeguimientoNCHelper_RenderError = p_PromptResult
    Exit Function

EH:
    p_Error = "modFrmNCAuditoriaSeguimientoNCHelper_RenderError: " & Err.Description
    modFrmNCAuditoriaSeguimientoNCHelper_RenderError = vbOK
End Function