Attribute VB_Name = "modFrmNCAuditoriaControlEficaciaAltaHelper"
Option Compare Database
Option Explicit

' =============================================================================
' Form helper for Form_FormNCAuditoriaControlEficaciaAlta (slice 10 of form-thin-helper-refactor).
'
' This helper owns the 1 message-class decision that previously lived inline
' in Form_FormNCAuditoriaControlEficaciaAlta.cls event handlers:
...
' Slice:    10 of 25 (ControlEficaciaAlta pair -- NCAuditoria side; companion is
'           modFrmNCProyectoControlEficaciaAltaHelper.bas on NCProyecto side).
' =============================================================================

' ----- Message contracts (byte-for-byte match with Test_modFrmNCAuditoriaControlEficaciaAltaHelper.bas) -----
...
'   blocks (mirrors modFrmNCAuditoriaControlEficaciaHelper_RenderError shape).
'
...
'         End If
'         pregunta = modFrmNCAuditoriaControlEficaciaAltaHelper_RenderError(m_Error, Err.Number, , m_HelperErr)
'
...
' -----------------------------------------------------------------------------
Public Function modFrmNCAuditoriaControlEficaciaAltaHelper_RenderError( _
    ...
    If Len(Trim$(p_ErrorText)) = 0 Then
        modFrmNCAuditoriaControlEficaciaAltaHelper_RenderError = PROMPT_ATOM
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ...
        Else
            style = vbCritical
        End If
        modFrmNCAuditoriaControlEficaciaAltaHelper_RenderError = MsgBox(p_ErrorText, style, MSG_ERROR_TITLE)
        Exit Function
    End If

    ' Test simulating user response.
    modFrmNCAuditoriaControlEficaciaAltaHelper_RenderError = p_PromptResult
    Exit Function

EH:
    p_Error = "modFrmNCAuditoriaControlEficaciaAltaHelper_RenderError: " & Err.Description
    modFrmNCAuditoriaControlEficaciaAltaHelper_RenderError = vbOK
End Function
