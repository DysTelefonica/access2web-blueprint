Attribute VB_Name = "Test_modFrmARProyectoDocumentosHelper"
Option Compare Database
Option Explicit

' =============================================================================
' Atoms for modFrmARProyectoDocumentosHelper (slice 6b of form-thin-helper-refactor).
'
...
'           inline in Form_FormARProyectoDocumentos.cls:
'             1. _RenderError                  -- boilerplate error handler (8 MsgBox)
'             2. _ConfirmarEliminacionDocumento -- delete-DocumentoProyecto confirmation (1 MsgBox)
'             3. _ValidarNombreDocumento        -- 4 validation branches in CambiarNombre
' Slice:    6b of 25 (Documentos group -- second batch: AR pair + FormAuditoriaDocumentos;
'           companion is modFrmARAuditoriaDocumentosHelper + modFrmAuditoriaDocumentosHelper).
...
'   - TbNCDocumentos columns: IDDocumento, IDNoConformidad, Documento
'     (table name verified via DocumentoProyecto.cls line 64:
'      constructor.getID("TbNCDocumentos", "IDDocumento", getdb(), m_Error))
'   - TbNoConformidades PK is IDNoConformidad (verified via slice 6a NCProyectoDocumentos test fixture).
' =============================================================================
...
' ----- Fixture IDs (>=900662100 reserved for this module, access-vba-tdd §1.7) -----
Private Const FIX_ID_NC As Long = 900662101
Private Const FIX_ID_DOC As Long = 900662102
...
' ConfirmarEliminacionDocumento path. Slice 6a NCProyectoDocumentos test uses the
' same fixture pattern (verified working).
Private Function EnsureNCFixtureForSlice6b(ByVal p_Db As DAO.Database, _
                                            ByVal p_IDNC As Long, _
                                            ByRef p_Error As String) As Boolean
    On Error GoTo EH
    EnsureNCFixtureForSlice6b = False
    p_Error = ""
    ...
    p_Db.Execute "INSERT INTO TbNoConformidades (IDNoConformidad, DESCRIPCION, ESTADO, FECHAAPERTURA) " & _
                 ...
                 "'NC fixture slice 6b AR proyecto documentos', " & _
                 "'REGISTRADA', #2026-06-26#)", dbFailOnError
    EnsureNCFixtureForSlice6b = True
    Exit Function
EH:
    p_Error = "EnsureNCFixtureForSlice6b: " & Err.Description
End Function
...
Private Function BuildDocumentoProyectoForSlice6b(ByVal p_IDDoc As Long, _
                                                  ByVal p_IDNC As Long, _
    ...
    Set BuildDocumentoProyectoForSlice6b = doc
End Function
...
' =============================================================================
Public Function Test_modFrmARProyectoDocumentosHelper_RenderError_Happy_NoModal_Atomic() As String
    ...
    returnCode = modFrmARProyectoDocumentosHelper_RenderError( _
        "Al btnExaminar_Click se ha producido el error n: 9" & vbNewLine & "Detalle: subscript out of range", _
        9, -1, helperErrMsg)

    If helperErrMsg <> "" Then
        Test_modFrmARProyectoDocumentosHelper_RenderError_Happy_NoModal_Atomic = _
...
EH:
    Test_modFrmARProyectoDocumentosHelper_RenderError_Happy_NoModal_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmARProyectoDocumentosHelper_RenderError_Happy_NoModal_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 2 - Sad: _RenderError with Err.Number=1000 (warning path).
' =============================================================================
Public Function Test_modFrmARProyectoDocumentosHelper_RenderError_Sad_WarningPath_Atomic() As String
    Dim logs As Collection
    Dim returnCode As Long
    Dim helperErrMsg As String
    ...
    If helperErrMsg <> "" Then
        Test_modFrmARProyectoDocumentosHelper_RenderError_Sad_WarningPath_Atomic = _
            TestHelper.BuildJsonFail("helperErrMsg should be empty (got='" & helperErrMsg & "')", logs)
        Exit Function
    End If
    TestHelper.AddLog logs, "Assert OK: helperErrMsg empty"
...
EH:
    Test_modFrmARProyectoDocumentosHelper_RenderError_Sad_WarningPath_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmARProyectoDocumentosHelper_RenderError_Sad_WarningPath_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 3 - Edge: _RenderError with empty m_ErrorText returns -1, no modal.
' =============================================================================
Public Function Test_modFrmARProyectoDocumentosHelper_RenderError_Edge_EmptyText_Atomic() As String
    Dim logs As Collection
    Dim returnCode As Long
    Dim helperErrMsg As String
    ...
    If helperErrMsg <> "" Then
        Test_modFrmARProyectoDocumentosHelper_RenderError_Edge_EmptyText_Atomic = _
            TestHelper.BuildJsonFail("helperErrMsg should be empty (got='" & helperErrMsg & "')", logs)
        Exit Function
    End If
    TestHelper.AddLog logs, "Assert OK: helperErrMsg empty"
...
EH:
    Test_modFrmARProyectoDocumentosHelper_RenderError_Edge_EmptyText_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmARProyectoDocumentosHelper_RenderError_Edge_EmptyText_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 4 - Adversarial: _RenderError with very long error text.
' =============================================================================
Public Function Test_modFrmARProyectoDocumentosHelper_RenderError_Adversarial_VeryLongText_Atomic() As String
    Dim logs As Collection
    Dim returnCode As Long
    Dim helperErrMsg As String
    ...
    If helperErrMsg <> "" Then
        Test_modFrmARProyectoDocumentosHelper_RenderError_Adversarial_VeryLongText_Atomic = _
            TestHelper.BuildJsonFail("helperErrMsg should be empty (got='" & helperErrMsg & "')", logs)
        Exit Function
    End If
    TestHelper.AddLog logs, "Assert OK: helperErrMsg empty"
...
EH:
    Test_modFrmARProyectoDocumentosHelper_RenderError_Adversarial_VeryLongText_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmARProyectoDocumentosHelper_RenderError_Adversarial_VeryLongText_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 5 - Happy: _ConfirmarEliminacionDocumento atom mode returns prompt text.
' =============================================================================
Public Function Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Happy_ReturnsPromptText_Atomic() As String
    Dim logs As Collection
    ...
    Set doc = BuildDocumentoProyectoForSlice6b(FIX_ID_DOC, FIX_ID_NC, "fixture_doc_eliminar.pdf")

    resultJson = modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento( _
        doc, Nothing, -1, helperErrMsg)

    If helperErrMsg <> "" Then
        Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Happy_ReturnsPromptText_Atomic = _
            TestHelper.BuildJsonFail("helperErrMsg should be empty (got='" & helperErrMsg & "')", logs)
        Exit Function
    End If
    TestHelper.AddLog logs, "Assert OK: helperErrMsg empty"
...
EH:
    Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Happy_ReturnsPromptText_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Happy_ReturnsPromptText_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 6 - Sad: _ConfirmarEliminacionDocumento with vbNo returns user_rejected.
' =============================================================================
Public Function Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Sad_UserRejected_Atomic() As String
    Dim logs As Collection
    Dim resultJson As String
    Dim helperErrMsg As String
    Dim doc As DocumentoProyecto
    ...
    If helperErrMsg <> "" Then
        Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Sad_UserRejected_Atomic = _
            TestHelper.BuildJsonFail("helperErrMsg should be empty (got='" & helperErrMsg & "')", logs)
        Exit Function
    End If
    TestHelper.AddLog logs, "Assert OK: helperErrMsg empty"
...
EH:
    Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Sad_UserRejected_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Sad_UserRejected_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 7 - Edge: _ConfirmarEliminacionDocumento with vbYes passes the gate.
' =============================================================================
Public Function Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Edge_UserConfirms_Atomic() As String
    Dim logs As Collection
    Dim resultJson As String
    Dim helperErrMsg As String
    ...
    If Not TestHelper.BeginTestSession(logs) Then
        Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Edge_UserConfirms_Atomic = _
            TestHelper.BuildJsonFail("BeginTestSession failed", logs)
        GoTo Cleanup
    End If
    Set db = getdb()
    If Not EnsureNCFixtureForSlice6b(db, FIX_ID_NC, helperErrMsg) Then
        Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Edge_UserConfirms_Atomic = _
            TestHelper.BuildJsonFail("EnsureNCFixtureForSlice6b: " & helperErrMsg, logs)
        GoTo Cleanup
    End If
...
EH:
    Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Edge_UserConfirms_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Edge_UserConfirms_Atomic: " & Err.Description, logs)
...
' =============================================================================
Public Function Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Adversarial_NothingDoc_Atomic() As String
    Dim logs As Collection
    Dim resultJson As String
    Dim helperErrMsg As String
    ...
    If InStr(1, resultJson, "Nothing", vbTextCompare) = 0 Then
        Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Adversarial_NothingDoc_Atomic = _
...
EH:
    Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Adversarial_NothingDoc_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmARProyectoDocumentosHelper_ConfirmarEliminacionDocumento_Adversarial_NothingDoc_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 9 - Happy: _ValidarNombreDocumento with unique new name returns ok=true.
' =============================================================================
Public Function Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Happy_UniqueName_Atomic() As String
    Dim logs As Collection
    Dim resultJson As String
    Dim helperErrMsg As String
    ...
    Set docActual = BuildDocumentoProyectoForSlice6b(FIX_ID_DOC, FIX_ID_NC, "original_doc.pdf")
    Set docOtro = BuildDocumentoProyectoForSlice6b(FIX_ID_DOC + 100, FIX_ID_NC, "otro_doc.pdf")
    ...
    resultJson = modFrmARProyectoDocumentosHelper_ValidarNombreDocumento( _
        "nuevo_nombre_unico.pdf", docActual, colDocs, -1, helperErrMsg)

    If helperErrMsg <> "" Then
        Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Happy_UniqueName_Atomic = _
            TestHelper.BuildJsonFail("helperErrMsg should be empty (got='" & helperErrMsg & "')", logs)
        Exit Function
    End If
    TestHelper.AddLog logs, "Assert OK: helperErrMsg empty"
...
EH:
    Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Happy_UniqueName_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Happy_UniqueName_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 10 - Sad: _ValidarNombreDocumento with empty new name returns MSG_NOMBRE_VACIO.
' =============================================================================
Public Function Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Sad_EmptyName_Atomic() As String
    Dim logs As Collection
    Dim resultJson As String
    Dim helperErrMsg As String
    Dim docActual As DocumentoProyecto
    ...
    If InStr(1, resultJson, MSG_NOMBRE_VACIO, vbTextCompare) = 0 Then
        Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Sad_EmptyName_Atomic = _
...
EH:
    Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Sad_EmptyName_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Sad_EmptyName_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 11 - Edge: _ValidarNombreDocumento with same-as-current returns MSG_NOMBRE_IGUAL.
' =============================================================================
Public Function Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Edge_SameName_Atomic() As String
    Dim logs As Collection
    Dim resultJson As String
    Dim helperErrMsg As String
    Dim docActual As DocumentoProyecto
    ...
    If InStr(1, resultJson, MSG_NOMBRE_IGUAL, vbTextCompare) = 0 Then
        Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Edge_SameName_Atomic = _
...
EH:
    Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Edge_SameName_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Edge_SameName_Atomic: " & Err.Description, logs)
End Function

' =============================================================================
' ATOM 12 - Adversarial: _ValidarNombreDocumento with duplicate in collection.
' =============================================================================
Public Function Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Adversarial_DuplicateName_Atomic() As String
    Dim logs As Collection
    Dim resultJson As String
    Dim helperErrMsg As String
    Dim docActual As DocumentoProyecto
    ...
    Set docActual = BuildDocumentoProyectoForSlice6b(FIX_ID_DOC, FIX_ID_NC, "original_doc.pdf")
    Set docOtro = BuildDocumentoProyectoForSlice6b(FIX_ID_DOC + 100, FIX_ID_NC, "nombre_que_ya_existe.pdf")
    ...
    If InStr(1, resultJson, MSG_NOMBRE_DUPLICADO, vbTextCompare) = 0 Then
        Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Adversarial_DuplicateName_Atomic = _
...
EH:
    Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Adversarial_DuplicateName_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmARProyectoDocumentosHelper_ValidarNombreDocumento_Adversarial_DuplicateName_Atomic: " & Err.Description, logs)
End Function