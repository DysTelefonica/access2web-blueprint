Attribute VB_Name = "Test_modFrmNCAuditoriaARHelper"
Option Compare Database
Option Explicit

' =============================================================================
' Atoms for modFrmNCAuditoriaARHelper (slice 5 of form-thin-helper-refactor).
'
...
'           inline in Form_FormNCAuditoriaAR.cls:
'             1. _RenderError          -- boilerplate error handler (~14 of 16 MsgBox)
'             2. _ConfirmarBorradoNC   -- MsgBox("¿Desea borrar la AR?") + InputBox(motivo)
...
' Slice:    5 of 27 (AR pair: NCAuditoriaAR; NCProyectoAR = 5a).
'           Mirror of slice 3 Acciones pair with name changes only.
' =============================================================================

' ----- Message contract (byte-for-byte match with production literals) -----
Private Const MSG_BORRADO_PROMPT As String = "MSG-AUDITORIA-AR-BORRADO: ¿Desea borrar la Acción Realizada de auditoría seleccionada?"
Private Const MSG_HABILITACION_PROMPT As String = "MSG-AUDITORIA-AR-HABILITACION: ¿Desea habilitar la no conformidad de auditoría seleccionada?"
Private Const MSG_MOTIVO_PROMPT As String = "MSG-AUDITORIA-AR-MOTIVO: Introduzca el motivo para marcar como borrada la no conformidad de auditoría"
...
' ----- Fixture IDs (>=900651000 reserved for this module, access-vba-tdd §1.7) -----
Private Const FIX_ID_NC As Long = 900651001
...
' Eliminar/Habilitar path. Mirrors the slice 3 NCAuditoriaAcciones test pattern
' but uses FIX_ID_NC in the 900651001 range (slice 5 range, distinct from slice 3's
' 900641001 and slice 5a's 900650001).
Private Function EnsureNCAuditoriaFixtureForSlice5(ByVal p_Db As DAO.Database, _
                                                    ByVal p_IDNC As Long, _
                                                    ByVal p_Codigo As String, _
                                                    ByVal p_Borrado As Boolean, _
                                                    ByRef p_Error As String) As Boolean
    On Error GoTo EH
    EnsureNCAuditoriaFixtureForSlice5 = False
    p_Error = ""
    If Not TableExistsInDb(p_Db, "TbNoConformidadesAuditoria") Then
        ...
        Exit Function
    End If
    p_Db.Execute "DELETE FROM TbNoConformidadesAuditoria WHERE IDNoConformidadAuditoria=" & p_IDNC, dbFailOnError
    p_Db.Execute "INSERT INTO TbNoConformidadesAuditoria (IDNoConformidadAuditoria, CodigoNoConformidadAuditoria, " & _
                 "EXPEDIENTE, DESCRIPCION, ESTADO, FECHAAPERTURA, " & _
                 "IDExpediente, CodExp, Nemotecnico, Juridica, JuridicaExp, " & _
                 "IDTipo, DetectadoPor, ENTIDADRESPONSABLE, CausaYAnalisRaiz, " & _
                 "RESPONSABLECALIDAD, RESPONSABLEIMPLANTACION, " & _
                 "RequiereControlEficacia, MotivoNoRequiereControlEficacia, " & _
                 "Borrado, MotivoBorrado, ACR) " & _
                 "VALUES (" & p_IDNC & ", " & _
                 "'" & Replace(p_Codigo, "'", "''") & "', " & _
                 "'TEST-EXP-SLICE5-AUD', " & _
                 "'NC auditoria fixture slice 5 AR', " & _
                 "'REGISTRADA', #2026-06-26#, " & _
                 "0, 'TEST-EXP-SLICE5-AUD', 'TEST-NEMOT-SLICE5-AUD', '', '', " & _
                 "0, 'TEST', 'TEST-ENT-SLICE5-AUD', 'TEST-CAUSA-SLICE5-AUD', " & _
                 "'TEST_RESPONSABLE_CALIDAD_SLICE5_AUD', 'TEST_RESPONSABLE_IMPL_SLICE5_AUD', " & _
                 "'No', NULL, " & _
                 IIf(p_Borrado, "True", "False") & ", " & _
                 IIf(p_Borrado, "'motivo fixture slice 5 aud'", "NULL") & ", " & _
                 "'')", dbFailOnError
    EnsureNCAuditoriaFixtureForSlice5 = True
    Exit Function
EH:
    p_Error = "EnsureNCAuditoriaFixtureForSlice5: " & Err.Description
End Function
...
'   IDNoConformidadAuditoria (or id), CodigoNoConformidadAuditoria, Descripcion,
'   CausaYAnalisRaiz, IDTipo, EntidadResponsable, FechaApertura,
'   RequiereControlEficacia, MotivoNoRequiereControlEficacia, ControlEficacia,
'   FechaPrevistaControlEficacia, ExpedienteObj, MotivoBorrado, Borrado.
Private Function BuildNCAuditoriaForSlice5(ByVal p_IDNC As Long, _
                                            ByVal p_Borrado As Boolean, _
                                            ByVal p_MotivoBorrado As String) As NCAuditoria
    Dim nc As NCAuditoria
    Set nc = New NCAuditoria
    nc.id = CStr(p_IDNC)
    nc.Numero = "TEST-COD-SLICE5-AUD-" & p_IDNC
    nc.Descripcion = "NC auditoria fixture slice 5 AR " & p_IDNC
    nc.CAUSARAIZ = "TEST-CAUSA-SLICE5-AUD"
    nc.Tipo = "1"
    nc.RESPONSABLEIMPLANTACION = "TEST-ENT-SLICE5-AUD"
    ...
    Set BuildNCAuditoriaForSlice5 = nc
End Function
...
' =============================================================================
Public Function Test_modFrmNCAuditoriaARHelper_RenderError_Happy_NoModal_Atomic() As String
    ...
    returnCode = modFrmNCAuditoriaARHelper_RenderError( _
        "Al ComandoRegistrar_Click se ha producido el error n: 9" & vbNewLine & "Detalle: subscript out of range", _
        9, promptResult, helperErrMsg)

    If helperErrMsg <> "" Then
        Test_modFrmNCAuditoriaARHelper_RenderError_Happy_NoModal_Atomic = _
...
EH:
    Test_modFrmNCAuditoriaARHelper_RenderError_Happy_NoModal_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaARHelper_RenderError_Happy_NoModal_Atomic: " & Err.Description, logs)
End Function
...
' =============================================================================
Public Function Test_modFrmNCAuditoriaARHelper_RenderError_Sad_WarningPath_Atomic() As String
    Dim logs As Collection
    Dim returnCode As Long
    Dim helperErrMsg As String
    Dim promptResult As Long
    ...
    returnCode = modFrmNCAuditoriaARHelper_RenderError( _
        "No hay cambios que guardar", _
        1000, promptResult, helperErrMsg)

    If helperErrMsg <> "" Then
        Test_modFrmNCAuditoriaARHelper_RenderError_Sad_WarningPath_Atomic = _
...
EH:
    Test_modFrmNCAuditoriaARHelper_RenderError_Sad_WarningPath_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaARHelper_RenderError_Sad_WarningPath_Atomic: " & Err.Description, logs)
End Function
...
' =============================================================================
Public Function Test_modFrmNCAuditoriaARHelper_RenderError_Edge_EmptyText_Atomic() As String
    Dim logs As Collection
    Dim returnCode As Long
    Dim helperErrMsg As String
    Dim promptResult As Long
    ...
    If helperErrMsg <> "" Then
        Test_modFrmNCAuditoriaARHelper_RenderError_Edge_EmptyText_Atomic = _
...
EH:
    Test_modFrmNCAuditoriaARHelper_RenderError_Edge_EmptyText_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaARHelper_RenderError_Edge_EmptyText_Atomic: " & Err.Description, logs)
End Function
...
' =============================================================================
Public Function Test_modFrmNCAuditoriaARHelper_RenderError_Adversarial_VeryLongText_Atomic() As String
    Dim logs As Collection
    Dim returnCode As Long
    Dim helperErrMsg As String
    Dim promptResult As Long
    ...
    If helperErrMsg <> "" Then
        Test_modFrmNCAuditoriaARHelper_RenderError_Adversarial_VeryLongText_Atomic = _
...
EH:
    Test_modFrmNCAuditoriaARHelper_RenderError_Adversarial_VeryLongText_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaARHelper_RenderError_Adversarial_VeryLongText_Atomic: " & Err.Description, logs)
End Function
...
' =============================================================================
Public Function Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Happy_ReturnsPromptText_Atomic() As String
    Dim logs As Collection
    ...
    If Not TestHelper.BeginTestSession(logs) Then
        Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Happy_ReturnsPromptText_Atomic = _
            TestHelper.BuildJsonFail("BeginTestSession failed", logs)
        Exit Function
    End If
    SetupTestContext

    Set ncParaBorrar = BuildNCAuditoriaForSlice5(FIX_ID_NC, False, "")
    promptResult = -1  ' atom mode
    motivo = ""

    jsonResult = modFrmNCAuditoriaARHelper_ConfirmarBorradoNC( _
...
EH:
    Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Happy_ReturnsPromptText_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Happy_ReturnsPromptText_Atomic: " & Err.Description, logs)
    Resume Cleanup
End Function
...
' =============================================================================
Public Function Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Sad_UserRejected_Atomic() As String
    Dim logs As Collection
    Dim jsonResult As String
    Dim promptResult As Long
    Dim motivo As String
    ...
    Set logs = TestHelper.NewLogs()
    If Not TestHelper.BeginTestSession(logs) Then
        Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Sad_UserRejected_Atomic = _
            TestHelper.BuildJsonFail("BeginTestSession failed", logs)
        Exit Function
    End If
    SetupTestContext
...
EH:
    Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Sad_UserRejected_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Sad_UserRejected_Atomic: " & Err.Description, logs)
    Resume Cleanup
End Function
...
' =============================================================================
Public Function Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Edge_EmptyMotivo_Atomic() As String
    Dim logs As Collection
    Dim jsonResult As String
    Dim promptResult As Long
    Dim motivo As String
    ...
    Set logs = TestHelper.NewLogs()
    If Not TestHelper.BeginTestSession(logs) Then
        Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Edge_EmptyMotivo_Atomic = _
            TestHelper.BuildJsonFail("BeginTestSession failed", logs)
        Exit Function
    End If
    SetupTestContext
...
EH:
    Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Edge_EmptyMotivo_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Edge_EmptyMotivo_Atomic: " & Err.Description, logs)
    Resume Cleanup
End Function
...
' =============================================================================
Public Function Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Adversarial_NothingNC_Atomic() As String
    Dim logs As Collection
    Dim jsonResult As String
    Dim promptResult As Long
    Dim motivo As String
    ...
    Set logs = TestHelper.NewLogs()
    If Not TestHelper.BeginTestSession(logs) Then
        Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Adversarial_NothingNC_Atomic = _
            TestHelper.BuildJsonFail("BeginTestSession failed", logs)
        Exit Function
    End If
    SetupTestContext
...
EH:
    Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Adversarial_NothingNC_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaARHelper_ConfirmarBorradoNC_Adversarial_NothingNC_Atomic: " & Err.Description, logs)
    Resume Cleanup
End Function
...
'   Atom asserts the helper returns the prompt-text contract via JSON value.
' =============================================================================
Public Function Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Happy_ReturnsPromptText_Atomic() As String
    Dim logs As Collection
    Dim jsonResult As String
    Dim promptResult As Long
    ...
    Set logs = TestHelper.NewLogs()
    If Not TestHelper.BeginTestSession(logs) Then
        Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Happy_ReturnsPromptText_Atomic = _
            TestHelper.BuildJsonFail("BeginTestSession failed", logs)
        Exit Function
    End If
    SetupTestContext

    Set ncParaHabilitar = BuildNCAuditoriaForSlice5(FIX_ID_NC, True, "motivo fixture")
    promptResult = -1  ' atom mode

    jsonResult = modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC( _
...
EH:
    Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Happy_ReturnsPromptText_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Happy_ReturnsPromptText_Atomic: " & Err.Description, logs)
    Resume Cleanup
End Function
...
' =============================================================================
Public Function Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Sad_UserRejected_Atomic() As String
    Dim logs As Collection
    Dim jsonResult As String
    Dim promptResult As Long
    Dim ncParaHabilitar As NCAuditoria
    ...
    Set logs = TestHelper.NewLogs()
    If Not TestHelper.BeginTestSession(logs) Then
        Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Sad_UserRejected_Atomic = _
            TestHelper.BuildJsonFail("BeginTestSession failed", logs)
        Exit Function
    End If
    SetupTestContext
...
EH:
    Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Sad_UserRejected_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Sad_UserRejected_Atomic: " & Err.Description, logs)
    Resume Cleanup
End Function
...
' =============================================================================
Public Function Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Edge_AlreadyEnabled_Atomic() As String
    Dim logs As Collection
    Dim jsonResult As String
    Dim promptResult As Long
    ...
    Set logs = TestHelper.NewLogs()
    If Not TestHelper.BeginTestSession(logs) Then
        Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Edge_AlreadyEnabled_Atomic = _
            TestHelper.BuildJsonFail("BeginTestSession failed", logs)
        Exit Function
    End If
    SetupTestContext

    Set ncNoBorrada = BuildNCAuditoriaForSlice5(FIX_ID_NC, False, "")  ' NOT borrada
...
EH:
    Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Edge_AlreadyEnabled_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Edge_AlreadyEnabled_Atomic: " & Err.Description, logs)
    Resume Cleanup
End Function
...
' =============================================================================
Public Function Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Adversarial_NothingNC_Atomic() As String
    Dim logs As Collection
    Dim jsonResult As String
    Dim promptResult As Long
    Dim helperErrMsg As String
    ...
    Set logs = TestHelper.NewLogs()
    If Not TestHelper.BeginTestSession(logs) Then
        Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Adversarial_NothingNC_Atomic = _
            TestHelper.BuildJsonFail("BeginTestSession failed", logs)
        Exit Function
    End If
    SetupTestContext
...
EH:
    Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Adversarial_NothingNC_Atomic = _
        TestHelper.BuildJsonFail("Test_modFrmNCAuditoriaARHelper_ConfirmarHabilitacionNC_Adversarial_NothingNC_Atomic: " & Err.Description, logs)
    Resume Cleanup
End Function