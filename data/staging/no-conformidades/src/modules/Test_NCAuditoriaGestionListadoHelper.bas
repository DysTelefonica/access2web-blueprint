...
' ============================================
' TEST TodosParametrosBusquedaVacios (audit helper pure function)
' ============================================
Public Function Test_AuditListadoHelper_TodosParametrosBusquedaVacios_AllEmpty_Atomic() As String
    Dim logs As Collection
    Dim assertError As String
    ...
    Set logs = TestHelper.NewLogs
    result = False
    Test_AuditListadoHelper_TodosParametrosBusquedaVacios_AllEmpty_Atomic = TestHelper.BuildJsonFail("test did not complete", logs)

    On Error GoTo HandleError

    result = NCAuditoriaGestionListadoHelper.TodosParametrosBusquedaVacios( _
                    "", "", "", "", "", "", "", "")
    If Not TestHelper.AssertTrue(result, "All-empty audit search panel must return True", logs, assertError) Then GoTo Fail
    TestHelper.AddLog logs, "Assert: all-empty inputs -> True"

    Test_AuditListadoHelper_TodosParametrosBusquedaVacios_AllEmpty_Atomic = TestHelper.BuildJsonOk(logs, "all_empty_true")
    GoTo Cleanup

Fail:
    Test_AuditListadoHelper_TodosParametrosBusquedaVacios_AllEmpty_Atomic = TestHelper.BuildJsonFail(assertError, logs)
    GoTo Cleanup
    Exit Function

HandleError:
    Test_AuditListadoHelper_TodosParametrosBusquedaVacios_AllEmpty_Atomic = TestHelper.BuildJsonFail("unexpected error: " & Err.Description, logs)
    Resume Cleanup
