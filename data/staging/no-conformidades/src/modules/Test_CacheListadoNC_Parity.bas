...
'
' Test scenarios (11 RED tests, all atomic). TEST 11/12/13 (form fallback
' empty/disabled/no-user) were form-coupled (opened Form_FormNCProyectoGestion
' via DoCmd.OpenForm acHidden and called frm.getNCsFiltrados). After the
' cache-form-filter-coverage refactor they are duplicated by helper tests in
' Test_FormHelper_Coverage.bas: SC-1.1/SC-1.3 cover the empty/disabled cache
' cases (strict superset — assert seeded ID + exactly 1 log row) and the new
' Test_FormHelper_Listing_NoUserResilient_Atomic covers the no-user case.
' Tests now exercise the helper directly, not the form, per the project's
' "no UI coupling in tests" rule.
...
'   11. ResponsableTelefonica exact excludes prefix-overlap (R1)
'
...
' ResponsableTelefonica exact-match test fixtures (R1)
Private Const TEST_ID_NC_RESPONSABLE_TELEFONICA_EXACT As Long = TEST_ID_NC_BASE + 41
Private Const TEST_ID_NC_RESPONSABLE_TELEFONICA_OVERLAP As Long = TEST_ID_NC_BASE + 42
...
' ============================================
' TEST 14 — ResponsableTelefonica exact excludes prefix-overlap
' ============================================
' R1 regression: SQL path now uses exact match (=) instead of LIKE '*val*'
' for ResponsableTelefonica (parity with fallback + form semantics).
' Seed NC1 with RT="FNHC-RT-EXACT", NC2 with RT="XFNHC-RT-EXACT" (prefix
' overlap). NC2 must be excluded by exact match (LIKE would include it).
' Added post-fix as regression coverage; pre-fix would be RED with containsOverlap=true.
' ============================================
Public Function Test_CacheListado_ResponsableTelefonica_ExactNoSubstring_Atomic() As String
    On Error GoTo EH
    ...
    Set logs = TestHelper.NewLogs
    If Not TestHelper.BeginTestSession(logs, sessionErr) Then
        Test_CacheListado_ResponsableTelefonica_ExactNoSubstring_Atomic = TestHelper.BuildJsonFail("TESTS BLOCKED: " & sessionErr, logs)
        Exit Function
    End If
    Set db = getdb()
    Call EnsureCacheListadoClean(db)
    ...
    idExact = TEST_ID_NC_RESPONSABLE_TELEFONICA_EXACT
    idOverlap = TEST_ID_NC_RESPONSABLE_TELEFONICA_OVERLAP

    ' Arrange: NC1 RT="FNHC-RT-EXACT", NC2 RT="XFNHC-RT-EXACT" (prefix overlap).
    ' EnsureNCFixture does not set ResponsableTelefonica, so UPDATE source NC
    ' explicitly. UpsertListado carries ResponsableTelefonica from source to cache.
    Call CleanupFixture(db, idExact, logs)
    Call CleanupFixture(db, idOverlap, logs)
    If Not EnsureNCFixture(db, idExact, "FNHC-RT-CODE-1", "Desc test rt exact", opErr) Then GoTo Fail
    If Not EnsureNCFixture(db, idOverlap, "FNHC-RT-CODE-2", "Desc test rt overlap", opErr) Then GoTo Fail
    db.Execute "UPDATE TbNoConformidades SET ResponsableTelefonica='FNHC-RT-EXACT' WHERE IDNoConformidad=" & idExact, dbFailOnError
    db.Execute "UPDATE TbNoConformidades SET ResponsableTelefonica='XFNHC-RT-EXACT' WHERE IDNoConformidad=" & idOverlap, dbFailOnError
    ok = SyncListado(db, idExact, opErr): If Not ok Then GoTo Fail
    ok = SyncListado(db, idOverlap, opErr): If Not ok Then GoTo Fail
    TestHelper.AddLog logs, "Arrange: NC1=" & idExact & " RT=FNHC-RT-EXACT, NC2=" & idOverlap & " RT=XFNHC-RT-EXACT"

    ' Act
    Set col = CacheNCProyecto.GetListadoFiltradoSQL(p_ResponsableTelefonica:="FNHC-RT-EXACT")
    TestHelper.AddLog logs, "Act: returned " & col.count & " items (exact match should include only NC1)"

    ' Assert: ONLY NC1 returned (NC2 excluded by exact match, would be included by LIKE '*val*')
    If col.count = 0 Then
        assertError = "Esperado >=1 resultado, obtenido 0"
        GoTo Fail
    End If
    ...
    If Not TestHelper.AssertTrue(containsExact, "NC1 (RT=FNHC-RT-EXACT) debe estar en el resultado", logs, assertError) Then GoTo Fail
    If Not TestHelper.AssertTrue(Not containsOverlap, "NC2 (RT=XFNHC-RT-EXACT) NO debe estar en el resultado (overlap excluded by exact match)", logs, assertError) Then GoTo Fail

    Test_CacheListado_ResponsableTelefonica_ExactNoSubstring_Atomic = TestHelper.BuildJsonOk(logs, "exact_only")
    GoTo Cleanup
Fail:
    Test_CacheListado_ResponsableTelefonica_ExactNoSubstring_Atomic = TestHelper.BuildJsonFail(assertError, logs)
    GoTo Cleanup
EH:
    Test_CacheListado_ResponsableTelefonica_ExactNoSubstring_Atomic = TestHelper.BuildJsonFail(Err.Description, logs)
Cleanup:
    On Error Resume Next
    If Not db Is Nothing Then
        Call CleanupFixture(db, idExact, logs)
