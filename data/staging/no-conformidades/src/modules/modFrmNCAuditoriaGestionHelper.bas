Attribute VB_Name = "modFrmNCAuditoriaGestionHelper"
Option Compare Database
Option Explicit

' =============================================================================
' Form helper for Form_FormNCAuditoriaGestion (slice 2b of form-thin-helper-refactor).
'
' This helper owns the 3 message-class decisions that previously lived inline
' in Form_FormNCAuditoriaGestion.cls event handlers:
...
' ----- Message contracts (byte-for-byte match with Test_modFrmNCAuditoriaGestionHelper.bas) -----
...
' -----------------------------------------------------------------------------
Public Function modFrmNCAuditoriaGestionHelper_RenderError( _
    ...
    If Len(Trim$(p_ErrorText)) = 0 Then
        modFrmNCAuditoriaGestionHelper_RenderError = PROMPT_ATOM
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ...
        Else
            style = vbCritical
        End If
        modFrmNCAuditoriaGestionHelper_RenderError = MsgBox(p_ErrorText, style, MSG_ERROR_TITLE)
        Exit Function
    End If

    ' Test simulating user response.
    modFrmNCAuditoriaGestionHelper_RenderError = p_PromptResult
    Exit Function

EH:
    p_Error = "modFrmNCAuditoriaGestionHelper_RenderError: " & Err.Description
    modFrmNCAuditoriaGestionHelper_RenderError = vbOK
End Function
...
' -----------------------------------------------------------------------------
Public Function modFrmNCAuditoriaGestionHelper_ConfirmarBorradoNC( _
    ...
    If p_NC Is Nothing Then
        modFrmNCAuditoriaGestionHelper_ConfirmarBorradoNC = BuildFail(logs, "ConfirmarBorradoNC: p_NC is Nothing")
        Exit Function
    End If

    ' Atom mode: return prompt text contract. No real modal.
    If p_PromptResult = PROMPT_ATOM Then
        modFrmNCAuditoriaGestionHelper_ConfirmarBorradoNC = BuildOk(logs, MSG_BORRADO_PROMPT)
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ' Production: show real MsgBox.
        respuesta = MsgBox( _
            "¿Desea marcar como borrado la no conformidad de auditoria seleccionada?", _
            vbExclamation + vbYesNo + vbDefaultButton2, MSG_BORRADO_TITLE)
        If respuesta <> VB_YES Then
            modFrmNCAuditoriaGestionHelper_ConfirmarBorradoNC = BuildOk(logs, "user_rejected")
            Exit Function
        End If
    ...
    If Len(Trim$(motivoFinal)) = 0 Then
        ...
        modFrmNCAuditoriaGestionHelper_ConfirmarBorradoNC = BuildFail(logs, p_Error)
        Exit Function
    End If
    ...
    modFrmNCAuditoriaGestionHelper_ConfirmarBorradoNC = BuildOk(logs, "deleted")
    Exit Function

EH:
    p_Error = "modFrmNCAuditoriaGestionHelper_ConfirmarBorradoNC: " & Err.Description
    modFrmNCAuditoriaGestionHelper_ConfirmarBorradoNC = BuildFail(logs, p_Error)
End Function
...
'   Returns: JSON string with {ok, value, error, logs}.
' -----------------------------------------------------------------------------
Public Function modFrmNCAuditoriaGestionHelper_ConfirmarHabilitacionNC( _
                                            ByRef p_NC As NCAuditoria, _
                                            Optional ByRef db As DAO.Database = Nothing, _
                                            Optional ByRef p_PromptResult As Long = 0, _
                                            Optional ByRef p_Error As String = "" _
    ...
    ' Adversarial: p_NC is Nothing.
    If p_NC Is Nothing Then
        modFrmNCAuditoriaGestionHelper_ConfirmarHabilitacionNC = BuildFail(logs, "ConfirmarHabilitacionNC: p_NC is Nothing")
        Exit Function
    End If

    ' Atom mode: return prompt text contract. No real modal.
    If p_PromptResult = PROMPT_ATOM Then
        modFrmNCAuditoriaGestionHelper_ConfirmarHabilitacionNC = BuildOk(logs, MSG_HABILITACION_PROMPT)
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ' Production: show real MsgBox.
        respuesta = MsgBox( _
            "¿Desea habilitar la no conformidad de auditoria seleccionada?", _
    ...
    If respuesta <> VB_YES Then
        modFrmNCAuditoriaGestionHelper_ConfirmarHabilitacionNC = BuildOk(logs, "user_rejected")
        Exit Function
    End If
    ...
    If p_NC.Borrado = False Then
        ...
        modFrmNCAuditoriaGestionHelper_ConfirmarHabilitacionNC = BuildFail(logs, p_Error)
        Exit Function
    End If

    ' Delegate to operations class (pure DAO).
    Dim m_NCAudOp As NCaUDITORIAOperaciones
    Set m_NCAudOp = New NCaUDITORIAOperaciones
    Set m_NCAudOp.nc = p_NC

    opErr = ""
    m_NCAudOp.Habilitar opErr
    If opErr <> "" Then
        p_Error = opErr
        modFrmNCAuditoriaGestionHelper_ConfirmarHabilitacionNC = BuildFail(logs, p_Error)
        Exit Function
    ...
    modFrmNCAuditoriaGestionHelper_ConfirmarHabilitacionNC = BuildOk(logs, "habilitada")
    Exit Function

EH:
    p_Error = "modFrmNCAuditoriaGestionHelper_ConfirmarHabilitacionNC: " & Err.Description
    modFrmNCAuditoriaGestionHelper_ConfirmarHabilitacionNC = BuildFail(logs, p_Error)
End Function
