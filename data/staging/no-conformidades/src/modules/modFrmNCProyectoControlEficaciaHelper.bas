Attribute VB_Name = "modFrmNCProyectoControlEficaciaHelper"
Option Compare Database
Option Explicit

' =============================================================================
' Form helper for Form_FormNCProyectoControlEficacia (slice 9 of form-thin-helper-refactor).
'
' This helper owns the 1 message-class decision that previously lived inline
' in Form_FormNCProyectoControlEficacia.cls event handlers:
...
' Slice:    9 of 25 (ControlEficacia pair -- NCProyecto side; companion is
'           modFrmNCAuditoriaControlEficaciaHelper.bas on NCAuditoria side).
' =============================================================================

' ----- Message contracts (byte-for-byte match with Test_modFrmNCProyectoControlEficaciaHelper.bas) -----
...
'         End If
'         pregunta = modFrmNCProyectoControlEficaciaHelper_RenderError(m_Error, Err.Number, , m_HelperErr)
'
...
' -----------------------------------------------------------------------------
Public Function modFrmNCProyectoControlEficaciaHelper_RenderError( _
    ...
    If Len(Trim$(p_ErrorText)) = 0 Then
        modFrmNCProyectoControlEficaciaHelper_RenderError = PROMPT_ATOM
        Exit Function
    End If
    ...
    If p_PromptResult = PROMPT_PRODUCTION Then
        ...
        Else
            style = vbCritical
        End If
        modFrmNCProyectoControlEficaciaHelper_RenderError = MsgBox(p_ErrorText, style, MSG_ERROR_TITLE)
        Exit Function
    End If

    ' Test simulating user response.
    modFrmNCProyectoControlEficaciaHelper_RenderError = p_PromptResult
    Exit Function

EH:
    p_Error = "modFrmNCProyectoControlEficaciaHelper_RenderError: " & Err.Description
    modFrmNCProyectoControlEficaciaHelper_RenderError = vbOK
End Function