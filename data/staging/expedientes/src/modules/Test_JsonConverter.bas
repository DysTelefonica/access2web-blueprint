...
' Boolean canonical literal: BuildJsonOk must emit "true"/"false" not
' "verdadero"/"falso" (the latter being LCase(CStr(value)) on a Spanish OS).
' Without this guard, generic JSON parsers rejecting the test payload break
' the whole e2e round-trip on non-English locales.
Public Function Test_JsonConverter_BuildJsonOk_SerializesTrueAsCanonical() As String
    Dim logs(0 To 2) As String
    logs(0) = "Arrange True value"
    Dim result As String
    result = BuildJsonOk(True, logs)
    If InStr(result, """value"":true") = 0 Then
        logs(1) = "Assert failed: expected canonical 'true'"
        Test_JsonConverter_BuildJsonOk_SerializesTrueAsCanonical = BuildJsonFail("expected ""value"":true in: " & result, logs)
    Else
        logs(1) = "Assert canonical 'true' present"
        Test_JsonConverter_BuildJsonOk_SerializesTrueAsCanonical = BuildJsonOk("ok", logs)
    End If
End Function

Public Function Test_JsonConverter_BuildJsonOk_SerializesFalseAsCanonical() As String
    Dim logs(0 To 2) As String
    logs(0) = "Arrange False value"
    Dim result As String
    result = BuildJsonOk(False, logs)
    If InStr(result, """value"":false") = 0 Then
        logs(1) = "Assert failed: expected canonical 'false'"
        Test_JsonConverter_BuildJsonOk_SerializesFalseAsCanonical = BuildJsonFail("expected ""value"":false in: " & result, logs)
    Else
        logs(1) = "Assert canonical 'false' present"
        Test_JsonConverter_BuildJsonOk_SerializesFalseAsCanonical = BuildJsonOk("ok", logs)
    End If
End Function
