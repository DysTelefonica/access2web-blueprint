...
Public Function LeeConfiguracionLocal( _
    ...
    ' Drop any cached connection so the next getdb() re-opens against the
    ' URL we are about to resolve. This guards re-config flows where
    ' ResetGlobals was not called (LeeConfiguracionLocal can be invoked
    ' standalone, e.g. by the stale-cache tests in Test_BackendCache).
    Call InvalidateBackendCache
...
' Invalidate the cached backend connection. Use this any time the active
' backend URL or selection has changed and the next getdb() must reopen
' against the new target (re-config flow, runtime URL override, etc.).
' Idempotent: safe to call when m_dbCached is already Nothing.
Public Sub InvalidateBackendCache()
    On Error Resume Next
    If Not m_dbCached Is Nothing Then m_dbCached.Close
    Set m_dbCached = Nothing
    On Error GoTo 0
...
' Public canonical accessor for the currently active backend KEY
' (PROD / SANDBOX / TEST). Returns the value stored in
' Application.TempVars("BackendActivo") by LeeConfiguracionLocal.
' Empty string if no config has been read.
Public Function GetActiveBackendKey() As String
    GetActiveBackendKey = Nz(Application.TempVars("BackendActivo"), "")
End Function

' TEST ONLY — not for production use. Allows tests to simulate a runtime
' change of m_ActiveBackendURL without going through LeeConfiguracionLocal
' (which would re-read the whole config and clear the cache on its own).
' Used by the stale-cache tests in Test_BackendCache to force getdb() to
' detect that the cached connection no longer matches the active URL.
Public Sub TestOnlyOverrideActiveBackendURL(ByVal p_URL As String)
    m_ActiveBackendURL = p_URL
End Sub
...
' TEST ONLY — checks if the cached backend connection is stale relative
' to the current testing backend URL (m_TestingBackendURL).
' Returns True if m_dbCached exists but points to a different file than
' the currently configured testing backend URL. Used by AssertSandboxBackend
' to detect stale cached connections when the sandbox URL changes.
Public Function TestOnlyIsSandboxCacheStale() As Boolean
    On Error GoTo EH
    
    If Not m_TestingMode Then
        TestOnlyIsSandboxCacheStale = False
        Exit Function
    End If
    
    If m_dbCached Is Nothing Then
        TestOnlyIsSandboxCacheStale = False
        Exit Function
    End If
    
    If Trim$(m_TestingBackendURL) = "" Then
        TestOnlyIsSandboxCacheStale = False
        Exit Function
    End If
    ...
    If Err.Number <> 0 Then
        ' Cached object is invalid — treat as stale
        Err.Clear
        TestOnlyIsSandboxCacheStale = True
        Exit Function
    End If
    On Error GoTo EH
    
    ' Compare cached connection's path against current testing backend URL
    If InStr(1, dummy, Trim$(m_TestingBackendURL), vbTextCompare) = 0 Then
        TestOnlyIsSandboxCacheStale = True
    Else
        TestOnlyIsSandboxCacheStale = False
    End If
...
Public Function getdb(Optional ByRef p_Error As String) As DAO.Database
    ...
    If Not m_dbCached Is Nothing Then
        ...
        dummy = m_dbCached.Name
        If Err.Number = 0 Then
            ' Stale-cache guard: the DAO object is alive, but the cached
            ' connection may point to a different file than the currently
            ' active backend URL (e.g. m_ActiveBackendURL changed at
            ' runtime, or m_TestingMode was toggled without ResetTestSession).
            ' Compare against the URL we are about to use; if it does not
            ' match, invalidate the cache so we reopen against the right one.
            Dim m_ExpectedUrl As String
            If m_TestingMode Then
                m_ExpectedUrl = Trim$(m_TestingBackendURL)
            Else
                m_ExpectedUrl = Trim$(m_ActiveBackendURL)
            End If
            If Len(m_ExpectedUrl) > 0 And InStr(1, dummy, m_ExpectedUrl, vbTextCompare) = 0 Then
                ' Cached connection is stale — drop and fall through to open fresh.
                Err.Clear
                On Error GoTo errores
                Set m_dbCached = Nothing
            Else
