...
' PRUEBA-003/REFAC-4b2b — Testable DAO adapter (closes #60)
' Same SQL path as getDatosArbol but returns Collection<ExpedienteSuministrador>
' (object DTOs with SetPropiedad populated), accepting an injected DAO.Database
' for sandbox-isolated tests. p_Error = "" on success; "" on failure means
' the collection may be partial/empty (mirrors the production contract).
' If p_Db Is Nothing, falls back to getdb() — same as getDatosArbol.
Public Function GetDatosArbolTestable( _
    ...
    Set rs = dbToUse.OpenRecordset(m_SQL, dbOpenSnapshot)

    Do While Not rs.EOF
        Set item = New ExpedienteSuministrador
        item.SetPropiedad "IDExpedienteSuministrador", CStr(Nz(rs!IDExpedienteSuministrador, ""))
        item.SetPropiedad "IDExpediente", CStr(p_IDExpediente)
        item.SetPropiedad "IDSuministrador", CStr(Nz(rs!IDSuministrador, ""))
        item.SetPropiedad "IdPadre", rs!IdPadre
        item.SetPropiedad "ContratistaPrincipal", CStr(Nz(rs!ContratistaPrincipal, "No"))
        item.SetPropiedad "SubContratista", CStr(Nz(rs!SubContratista, "No"))
        item.SetPropiedad "Descripcon", CStr(Nz(rs!Descripcon, ""))
        col.Add item
        rs.MoveNext
    Loop
...
errores:
    If Err.Number <> 1000 Then p_Error = "GetDatosArbolTestable: " & Err.Description
    On Error Resume Next
    If Not rs Is Nothing Then rs.Close
    Set rs = Nothing
    Set GetDatosArbolTestable = New Collection
End Function
