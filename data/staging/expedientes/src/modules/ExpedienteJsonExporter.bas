...
' ------------------------------------------------------------------------------------------
' E2E Family Export: parameterized QueryDef helpers (mirrors E2EFamilyExportHelper.bas).
' Duplicated as Private to keep E2E export modules decoupled and stable.
' ------------------------------------------------------------------------------------------
...
Public Function GenerarJsonE2ECanonicoPorLista(ByVal p_IDExpedientesCsv As String, Optional ByRef p_Error As String) As String
    ...
    Set db = getdb(p_Error)
    If p_Error <> "" Or db Is Nothing Then Exit Function

    GenerarJsonE2ECanonicoPorLista = GenerateE2ECanonicalJsonForCsv(db, p_IDExpedientesCsv, Null, p_Error)
...
' ------------------------------------------------------------------------------------------
' Genera JSON E2E canónico expandiendo la lista CSV a la familia completa (root + descendants).
' Asigna OrdinalE2E faltantes de forma transaccional; rollback si falla.
' ------------------------------------------------------------------------------------------
Public Function GenerarJsonE2ECanonicoPorListaConFamilia(ByVal p_IDExpedientesCsv As String, Optional ByRef p_Error As String) As String
    On Error GoTo ErrorHandler

    Dim db As DAO.Database
    Dim expandedCsv As String
    Dim ws As DAO.Workspace
    Dim transActive As Boolean

    p_Error = ""
    GenerarJsonE2ECanonicoPorListaConFamilia = ""
    transActive = False
    Set ws = Nothing
    ...
    expandedCsv = E2EFamilyExport_ExpandCsvToFamilyCsv(db, p_IDExpedientesCsv, p_Error)
    If p_Error <> "" Then GoTo SafeExit

    Set ws = DBEngine.Workspaces(0)
    ws.BeginTrans
    transActive = True

    If Not E2EFamilyExport_AssignMissingOrdinals(db, expandedCsv, p_Error) Then GoTo SafeExit
    If p_Error <> "" Then GoTo SafeExit

    GenerarJsonE2ECanonicoPorListaConFamilia = GenerateE2ECanonicalJsonForCsv(db, expandedCsv, Null, p_Error)
    If p_Error <> "" Then GenerarJsonE2ECanonicoPorListaConFamilia = ""
...
ErrorHandler:
    If p_Error = "" Then p_Error = "GenerarJsonE2ECanonicoPorListaConFamilia: " & Err.Description
    GenerarJsonE2ECanonicoPorListaConFamilia = ""
    Resume SafeExit
End Function

' ------------------------------------------------------------------------------------------
' Wrapper batch: delegates to the family core and preserves IDExportacion / IDBatch in meta.
' ------------------------------------------------------------------------------------------
Public Function GenerarJsonE2ECanonicoPorListaConFamiliaParaBatch( _
    ...
    p_Error = ""
    GenerarJsonE2ECanonicoPorListaConFamiliaParaBatch = ""
    transActive = False
    Set ws = Nothing
    ...
    GenerarJsonE2ECanonicoPorListaConFamiliaParaBatch = GenerateE2ECanonicalJsonForCsv(db, expandedCsv, CLng(p_IDBatch), p_Error)
    If p_Error <> "" Then GenerarJsonE2ECanonicoPorListaConFamiliaParaBatch = ""
...
ErrorHandler:
    If p_Error = "" Then p_Error = "GenerarJsonE2ECanonicoPorListaConFamiliaParaBatch: " & Err.Description
    GenerarJsonE2ECanonicoPorListaConFamiliaParaBatch = ""
    Resume SafeExit
End Function
...
Private Function GetE2EBatchMetadataDictionary(ByVal p_Total As Long, ByVal p_IDBatch As Long) As Object
    Dim metadata As Object
    Set metadata = CreateObject("Scripting.Dictionary")
    metadata.Add "apiVersion", "1.0"
    metadata.Add "generatedAt", Format$(Now, "yyyy-mm-dd\Thh:nn:ss") & ".000Z"
    ...
    metadata.Add "user", Environ$("USERNAME")
    metadata.Add "totalExpedientes", p_Total
    metadata.Add "IDExportacion", p_IDBatch
    metadata.Add "IDBatch", p_IDBatch
    Set GetE2EBatchMetadataDictionary = metadata
End Function

' ------------------------------------------------------------------------------------------
' Helper privado compartido: genera JSON E2E canónico a partir de un CSV de IDs.
' Usa QueryDef parametrizado (A.1) con OR-chain predicate. Si p_IDBatch viene provisto,
' emite meta con IDExportacion/IDBatch (ruta batch); si no, meta estándar.
' ------------------------------------------------------------------------------------------
Private Function GenerateE2ECanonicalJsonForCsv( _
    ByVal p_Db As DAO.Database, _
    ByVal p_IDExpedientesCsv As String, _
    Optional ByVal p_IDBatch As Variant, _
    Optional ByRef p_Error As String) As String
    ...
    p_Error = ""
    GenerateE2ECanonicalJsonForCsv = ""

    Set ids = ParseCsvIds(p_IDExpedientesCsv)
    If ids.Count = 0 Then
        p_Error = "GenerateE2ECanonicalJsonForCsv: lista de IDs vacía o inválida"
        Exit Function
    End If

    Set qdf = p_Db.CreateQueryDef("", _
        "PARAMETERS " & BuildIdParameterDeclarations(ids) & "; " & _
        "SELECT TbExpedientes.*, " & _
        ...
        "WHERE (" & BuildParameterizedIdPredicate(ids, "IDExpediente") & ") " & _
    ...
    Do While Not rs.EOF
        Set rowDict = BuildE2ECanonicalRowDict(p_Db, rs)
        dataRows.Add rowDict
        total = total + 1
        rs.MoveNext
    Loop
    ...
    Set finalDict = CreateObject("Scripting.Dictionary")
    If IsMissing(p_IDBatch) Or IsNull(p_IDBatch) Then
        finalDict.Add "meta", GetE2ECanonicalMetadataDictionary(total)
    Else
        finalDict.Add "meta", GetE2EBatchMetadataDictionary(total, CLng(p_IDBatch))
    End If
    finalDict.Add "data", dataRows

    GenerateE2ECanonicalJsonForCsv = JsonConverter.ConvertToJson(finalDict, "  ")
...
ErrorHandler:
    If p_Error = "" Then p_Error = "GenerateE2ECanonicalJsonForCsv: " & Err.Description
    GenerateE2ECanonicalJsonForCsv = ""
    Resume SafeExit
End Function
...
' ------------------------------------------------------------------------------------------
Public Function UnescapeUnicode(ByVal str As String) As String
    Dim regex As Object
