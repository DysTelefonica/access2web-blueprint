...
Public Function NombrePestanaDesdeSourceObject( _
                                    ByVal p_SourceObject As String _
                                    ) As String
    Select Case Nz(p_SourceObject, "")
        Case "tabGeneral", "FormExpedienteGeneral"
            NombrePestanaDesdeSourceObject = "tabGeneral"
        Case "tabFechas", "FormExpedienteFechas"
            NombrePestanaDesdeSourceObject = "tabFechas"
        Case "tabEntidades", "FormExpedienteEntidades"
            NombrePestanaDesdeSourceObject = "tabEntidades"
        Case "tabSuministradores", "FormExpedienteSuministradores"
            NombrePestanaDesdeSourceObject = "tabSuministradores"
        Case "tabAnexos", "FormExpedienteDocumentacion"
            NombrePestanaDesdeSourceObject = "tabAnexos"
        Case "tabHitos", "FormExpedienteHitos"
            NombrePestanaDesdeSourceObject = "tabHitos"
        Case "tabModificados", "FormExpedienteModificados"
            NombrePestanaDesdeSourceObject = "tabModificados"
    End Select
End Function

Public Function EsPestanaGeneralOFechas( _
                                    ByVal p_NombrePestana As String _
                                    ) As Boolean
    Select Case NombrePestanaDesdeSourceObject(p_NombrePestana)
        Case "tabGeneral", "tabFechas"
            EsPestanaGeneralOFechas = True
    End Select
End Function

Public Function DebeGuardarPestanaGeneralFechas( _
                                    ByVal p_NombrePestana As String, _
                                    ByVal p_EsModoEdicion As Boolean, _
                                    ByVal p_AllowEdits As Boolean, _
                                    ByVal p_HayCambios As Boolean _
                                    ) As Boolean
    If Not p_EsModoEdicion Then Exit Function
    If Not p_AllowEdits Then Exit Function
    If Not p_HayCambios Then Exit Function

    DebeGuardarPestanaGeneralFechas = EsPestanaGeneralOFechas(p_NombrePestana)
End Function

Public Function HayCambiosFormularioPestana( _
                                    frm As Form, _
                                    Optional ByRef p_Error As String _
                                    ) As Boolean
    Dim m_SnapshotActual As String
    Dim m_SnapshotInicial As String

    On Error GoTo errores

    If frm Is Nothing Then Exit Function

    m_SnapshotInicial = Nz(CallByName(frm, "m_SnapshotAlInicio", VbGet), "")
    m_SnapshotActual = getSnapshot(frm:=frm, p_Error:=p_Error)
    If p_Error <> "" Then
        Err.Raise 1000
    End If

    HayCambiosFormularioPestana = (m_SnapshotActual <> m_SnapshotInicial)
    Exit Function

errores:
    If Err.Number <> 1000 Then
        p_Error = "El método HayCambiosFormularioPestana ha devuelto el error: " & vbNewLine & Err.Description
    End If
End Function

Public Function ActualizarSnapshotFormularioPestana( _
                                    frm As Form, _
                                    Optional ByRef p_Error As String _
                                    ) As String
    Dim m_SnapshotActual As String
    ...
    CallByName frm, "m_SnapshotAlInicio", VbLet, m_SnapshotActual
    ActualizarSnapshotFormularioPestana = "1"
    Exit Function

errores:
    If Err.Number <> 1000 Then
        p_Error = "El método ActualizarSnapshotFormularioPestana ha devuelto el error: " & vbNewLine & Err.Description
    End If
End Function
...
Public Function getNombrePestana( _
    ...
    On Error GoTo errores
    getNombrePestana = NombrePestanaDesdeSourceObject(frm.Name)
    If getNombrePestana = "" Then
...
Public Function GuardarPestanaFechasValores( _
                                        ByVal p_IDExpediente As String, _
                                        ByVal p_FechaInicioLicitacion As Variant, _
                                        ByVal p_FechaAdjudicacion As Variant, _
                                        Optional ByRef p_Error As String _
                                        ) As String
    Dim rcdDatos As DAO.Recordset
    Dim m_SQL As String
    ...
    Set dbUse = getdb(p_Error)
    If dbUse Is Nothing Then
        If p_Error = "" Then p_Error = "No se pudo abrir la base de datos"
        Err.Raise 1000
    End If
    ...
    If IsDate(p_FechaInicioLicitacion) Then
        rcdDatos.Fields("FECHAINICIOLICITACION").value = CDate(p_FechaInicioLicitacion)
    Else
        rcdDatos.Fields("FECHAINICIOLICITACION").value = Null
    End If

    If IsDate(p_FechaAdjudicacion) Then
        rcdDatos.Fields("FECHAADJUDICACION").value = CDate(p_FechaAdjudicacion)
    Else
        rcdDatos.Fields("FECHAADJUDICACION").value = Null
    End If
...
errores:
    ...
    On Error Resume Next
    If Not rcdDatos Is Nothing Then
        rcdDatos.CancelUpdate
        rcdDatos.Close
    End If
    Set rcdDatos = Nothing
    If m_ErrNumber <> 1000 Then
        p_Error = "GuardarPestanaFechasValores: " & m_ErrDescription
    End If
End Function
...
Public Function GuardarPestanaGeneralValores( _
                                        ByVal p_IDExpediente As String, _
                                        ByVal p_Titulo As String, _
                                        ByVal p_Ambito As String, _
                                        Optional ByRef p_Error As String _
                                        ) As String
    Dim rcdDatos As DAO.Recordset
    Dim m_SQL As String
    ...
    rcdDatos.Edit
    rcdDatos.Fields("Titulo").value = p_Titulo
    rcdDatos.Fields("Ambito").value = Nz(p_Ambito, "")
    rcdDatos.Update
    rcdDatos.Close
    Set rcdDatos = Nothing

    GuardarPestanaGeneralValores = "1"
    Exit Function
...
errores:
    ...
    Set rcdDatos = Nothing
    If m_ErrNumber <> 1000 Then
        p_Error = "GuardarPestanaGeneralValores: " & m_ErrDescription
    End If
End Function
