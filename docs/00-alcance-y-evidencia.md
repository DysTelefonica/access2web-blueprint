...
|---|---|---|
| 1 | Lanzadera | Mapeo resuelto. |
| 2 | Gestion_Riesgos | Mapeo resuelto. |
| 3 | No_Conformidades | Mapeo resuelto; inventario Dysflow pendiente. |
| 4 | Condor | Frontend localizado; backend y `.dysflow/project.json` pendientes. |
| 5 | HPS_Solicitudes | Documentación localizada; checkout/binary no identificados. |
| 6 | HPS | Mapeo resuelto. |
| 7 | Brass | Frontend localizado; backend y `.dysflow/project.json` pendientes. |
| 8 | Expedientes | Mapeo resuelto con aviso de `path-mismatch` en Dysflow. |
...
3. Inspección Dysflow en vivo, **solo lectura** (`get_capabilities`, `list_objects`, `list_tables`).
4. CodeGraph y CodeGraph-VBA como apoyo de navegación por símbolos.
5. Engram — **contexto histórico únicamente**; nunca se eleva a hecho actual.
