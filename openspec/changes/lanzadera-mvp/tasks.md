...
> Change: `lanzadera-mvp` · Project: `access2web-blueprint` · strict_tdd: true · pytest: `pytest --cov=platform --cov-fail-under=85`
...
|------|------|-----------|----------------------|-----------------|-------------------|
| 1 | Phase 0 — platform scaffold + CI gates | PR 1 (feat/0001-platform-skeleton) | `pytest --cov=platform --cov-fail-under=85` | `docker compose up -d postgres` | Drop schema + remove `platform/` directory |
| 2 | Phase 1 — domain entities | PR 2 (feat/0002-domain-entities) | `pytest platform/tests/lanzadera/` | Same compose | Delete domain entities |
...
| 4 | Phase 3 — application layer (CRITICAL_HELPERS) | PR 4 (feat/0004-application-use-cases) | `pytest --cov=platform --cov-fail-under=85` | Same compose | Revert to stub use cases |
| 5 | Phase 5 — reset flow + lockout (D90, D38–D40) | PR 5 (feat/0005-reset-lockout) | `pytest platform/tests/lanzadera/auth/` | Same compose | Rollback token table rows |
| 6 | Phase 4 — adapters (persistence, crypto, notification, cache, location, bootstrap) | PR 6 (feat/0006-adapters) | `pytest platform/tests/lanzadera/` | Same compose | Delete adapter files |
| 7 | Phase 5 — delivery HTMX + CLI | PR 7 (feat/0007-delivery-cli) | `pytest platform/tests/lanzadera/delivery/` | `docker compose up` | Remove delivery routes |
...
- [ ] 0.1 Create `platform/` directory tree: `src/modules/lanzadera/{domain,ports,application,adapters,di,delivery}`, `migrations/versions/`, `tests/lanzadera/`, `scripts/`, `src/shared/`
- [ ] 0.2 Create `platform/pyproject.toml` with exact pins: `argon2-cffi==25.1.0`, `sqlalchemy==2.0.*`, `alembic==1.13.*`, `asyncpg==0.30.*`, `fastapi==0.119.*`, `jinja2==3.1.*`, `httpx==0.28.*`, `cachetools==5.*`, `ruff==0.15.21`, `mypy==1.13.0`; `pytest-cov`; `[tool.coverage]` with `platform/` as source
- [ ] 0.3 Create `platform/Dockerfile` with `python:3.12-slim-bookworm@sha256:<digest>`; multi-stage build; pin every `FROM` digest
...
- [ ] 0.9 Create `scripts/check_layers.py` from `deterministic-quality-harness/assets/`; set `ROOT_PACKAGE = "platform.src.modules"`, `PURE_LAYERS = {"domain","ports","application"}`, `ALLOWED_IMPORTS` per DA-1; create `tests/lanzadera/test_layers_wiring.py`
...
- [ ] 0.12 Create `scripts/check_pr_size.py` and `scripts/check_branch_name.py` per QC-6; `scripts/pytest_plugin/coverage_gate.py` declaring `CRITICAL_HELPERS = ["hash_password","verify_password","issue_reset_token","consume_reset_token"]`
- [ ] 0.13 Create `platform/.python-version` = `3.12`; `.gitignore`, `.dockerignore` covering `__pycache__`, `.ruff_cache`, `.mypy_cache`, `.pytest_cache`, `.venv*`, `secrets/`
- [ ] 0.14 Create `platform/src/__init__.py`, `platform/src/shared/__init__.py`, `platform/src/modules/__init__.py`, `platform/src/modules/lanzadera/__init__.py`, `platform/src/main.py` (FastAPI app stub with `get_current_admin` dependency stub)

## Phase 1: Domain Layer (pure, framework-free)

- [ ] 1.1 Create `platform/src/modules/lanzadera/domain/user.py` with `User` entity: `id: UUID`, `email: str`, `name: str`, `dni_encrypted: bytes`, `password_hash: str | None`, `status: UserStatus` (ENUM `active,disabled,password_reset_required,locked`), `failed_attempts: int`, `last_login_at: datetime | None`, `created_at`, `updated_at`; `UserStatus` as `StrEnum`; domain invariants: email normalised lowercase, non-empty name
- [ ] 1.2 Create `platform/src/modules/lanzadera/domain/app.py` with `App` entity: `id: int`, `name: str`, `short_code: str`, `deployment_topology: AppTopology` (ENUM `central,office-nas`), `requires_office_presence: bool`, `registration_status: AppRegistrationStatus` (ENUM `pending,active,retired`), `created_at`, `updated_at`
- [ ] 1.3 Create `platform/src/modules/lanzadera/domain/profile.py` with `Profile` entity: `id: UUID`, `app_id: int`, `code: str`, `name: str`, `capabilities: dict` (JSONB shape `{string: string|number|boolean}`), `active: bool`, `created_at`, `updated_at`
- [ ] 1.4 Create `platform/src/modules/lanzadera/domain/assignment.py` with `Assignment` entity: `id: UUID`, `user_id: UUID`, `app_id: int`, `profile_id: UUID`, `granted_by: UUID | None`, `granted_at: datetime`, `revoked_at: datetime | None`
- [ ] 1.5 Create `platform/src/modules/lanzadera/domain/reset_token.py` with `ResetToken` value object: `id: UUID`, `user_id: UUID`, `token_hash: str`, `expires_at: datetime`, `consumed_at: datetime | None`, `superseded_at: datetime | None`, `created_at`
- [ ] 1.6 Create `platform/src/modules/lanzadera/domain/global_admin.py` with `GlobalAdmin` value object: `user_id: UUID`
- [ ] 1.7 Create `platform/src/modules/lanzadera/domain/audit_event.py` with `AuditEvent` entity: `id: UUID`, `event_type: str`, `actor_id: UUID | None`, `target_id: str`, `module: str`, `result: str`, `correlation_id: UUID`, `payload: dict`, `created_at`; **no** telemetry fields (DA-11)
- [ ] 1.8 Create `platform/src/modules/lanzadera/domain/legacy_role_map.py` (DA-12, closes G-2): map of 7 legacy flags (`Administrador`, `Calidad`, `CalidadAvisos`, `Técnico`, `Economía`, `Secretaría`, `SinAcceso`) to profile codes; `SinAcceso` exclusive cortocircuito; `DEFAULT` for all-NULL; unit-tested against 32-case matrix (H11); **Note**: G-2 capability set per app is ABIERTO — seed with `capabilities = {}` or minimal shape; product must confirm canonical set before 0003
- [ ] 1.9 Create `platform/src/modules/lanzadera/domain/session.py` with `Session` entity: `id: UUID`, `user_id: UUID`, `created_at`, `expires_at`; `LockoutPolicy` value object: `threshold: int` (default 5), `duration_seconds: int` (default 3600)
...
- [ ] 3.1 Create `platform/src/modules/lanzadera/ports/user_repository.py` Protocol: `get_by_email`, `create`, `update_status`, `set_password_and_activate`, `list_all`, `get_by_id`; `platform/src/modules/lanzadera/ports/app_repository.py`: `get_by_id`, `list_active`, `list_visible_to`; `platform/src/modules/lanzadera/ports/profile_repository.py`: `list_for_app`, `get_by_code`, `create`, `set_active`; `platform/src/modules/lanzadera/ports/assignment_repository.py`: `create`, `list_for_user`, `list_for_app`, `effective_permissions`; `platform/src/modules/lanzadera/ports/reset_token_repository.py`: `insert`, `find_unused`, `mark_consumed`, `mark_superseded`, `purge_expired`; `platform/src/modules/lanzadera/ports/global_admin_repository.py`: `list_all`, `is_global_admin`, `is_there_any`, `grant`, `revoke`; `platform/src/modules/lanzadera/ports/credential_hasher.py`: `hash_password`, `verify_password`; `platform/src/modules/lanzadera/ports/notification_delivery.py`: `send(to, subject, body) -> None`; `platform/src/modules/lanzadera/ports/location.py`: `is_user_in_office`; `platform/src/modules/lanzadera/ports/cache.py`: `get`, `set`, `invalidate`; `platform/src/modules/lanzadera/ports/audit_log.py`: `append`, `list_for_actor`; `platform/src/modules/lanzadera/ports/secret_manager.py`: `get`; `platform/src/modules/lanzadera/ports/bootstrap_admin_source.py`: `list_initial_emails`
...
- [ ] 3.4 Implement `hash_password` / `verify_password` in `platform/src/modules/lanzadera/application/credential_helpers.py` with `argon2-cffi==25.1.0` RFC_9106_LOW_MEMORY profile (DA-2)
...
- [ ] 3.7 Implement `issue_reset_token` and `consume_reset_token` in `platform/src/modules/lanzadera/application/`; token hash via `hashlib.sha256` (internal only, not exported); atomic transaction (DA-4, D90)
- [ ] 3.8 Implement `platform/src/modules/lanzadera/application/create_user.py`, `disable_user.py`, `grant_global_admin.py`, `revoke_global_admin.py`, `assign_profile.py`, `list_effective_apps.py`, `audit_append.py`, `bootstrap_global_admins.py`, `set_password.py` (CLI bootstrap, D91)

## Phase 4: Adapters — Driven implementations

- [ ] 4.1 Create `platform/src/modules/lanzadera/adapters/postgres/session.py`: `async_session_factory` from `asyncpg` + SQLAlchemy 2.0 Core; bound to `DATABASE_URL` from composition root
- [ ] 4.2 Create `platform/src/modules/lanzadera/adapters/postgres/user_repository_pg.py`: implements `UserRepositoryPort`; `dni_encrypted` encrypted via `NationalIdCipher` before INSERT; `update_status` for lockout transitions; `set_password_and_activate` for `consume_reset_token`
- [ ] 4.3 Create `platform/src/modules/lanzadera/adapters/postgres/app_repository_pg.py`, `profile_repository_pg.py`, `assignment_repository_pg.py`, `reset_token_repository_pg.py`, `global_admin_repository_pg.py`, `audit_log_pg.py`; `audit_log_pg` appends in the **same transaction** as the calling use case (DA-11)
- [ ] 4.4 Create `platform/src/modules/lanzadera/adapters/postgres/tests/` with in-memory fakes for all repo ports; implement contract tests: `test_user_repository_contract.py`, `test_app_repository_contract.py`, `test_profile_repository_contract.py`, `test_assignment_repository_contract.py`
- [ ] 4.5 Create `platform/src/modules/lanzadera/adapters/crypto/credential_hasher_argon2id.py`: implements `CredentialHasherPort`; delegates to `application/credential_helpers.py`
- [ ] 4.6 Create `platform/src/modules/lanzadera/adapters/crypto/national_id_cipher.py`: AES-GCM via `SecretManagerPort`; `encrypt(plain: str) -> bytes`; `decrypt(ciphertext: bytes) -> str`; key never in logs/CLI args (CA-S4)
- [ ] 4.7 Create `platform/src/modules/lanzadera/adapters/notification/mail_queue_table_adapter.py`: implements `NotificationDeliveryPort`; inserts into `mail_outbox` with `status='pending'` (DA-10; G-5 ABIERTO — adapter v1, SMTP real deferred to P20)
- [ ] 4.8 Create `platform/src/modules/lanzadera/adapters/location/assume_in_office_adapter.py`: `is_user_in_office -> True` always (DA-9, H12); **RED**: `tests/lanzadera/test_assume_in_office.py` asserts explicit stub return
- [ ] 4.9 Create `platform/src/modules/lanzadera/adapters/cache/ttl_cache_adapter.py`: implements `CachePort` using `cachetools.TTLCache`; candidates: `apps.list_active`, `profiles.list_for_app`, `assignments.effective_permissions`; TTLs from env vars; invalidation on mutation
- [ ] 4.10 Create `platform/src/modules/lanzadera/adapters/bootstrap/env_admin_source_adapter.py`: reads `GLOBAL_ADMIN_EMAILS`, splits by `;`, returns list; empty if unset (DA-6); **RED**: `tests/lanzadera/test_bootstrap_adapter.py` — idempotency, unset env, audit emission
- [ ] 4.11 Create `platform/src/modules/lanzadera/adapters/secrets/env_secret_manager_adapter.py`: `get(key: str) -> str`; reads from `PLATFORM_SECRET_KEY` env var or generates/persists on first boot (MVP); production swapped by Vault adapter without touching domain
- [ ] 4.12 Create `platform/src/modules/lanzadera/di/container.py`: composition root wiring all ports to their adapters; FastAPI `Depends()` declarations for every port
- [ ] 4.13 Create `platform/src/modules/lanzadera/di/bootstrap.py`: calls `bootstrap_global_admins` on process start; calls `reset_tokens.purge_expired` in background; idempotent (DA-6)
...
- [ ] 5.2 Create `platform/src/modules/lanzadera/delivery/http/templates/async_jinja/` for: `login.html`, `reset_request.html`, `reset_form.html`, `admin/users.html`, `admin/apps.html`, `admin/assignments.html`, `admin/audit.html`; Mistica token CSS vars for brand consistency
...
- [ ] 5.4 Create `platform/src/modules/lanzadera/delivery/cli/platform_user.py`: subcommands `set-password`, `grant-global-admin`, `revoke-global-admin`, `list-apps`, `assign-profile`; admin-only gate via `is_global_admin` check; destructive commands (`revoke`) prompt explicit confirmation (D26); secrets via env var, never CLI args (CA-S4)
...
- [ ] 6.1 Create `tests/lanzadera/auth/test_no_legacy_compat.py` (DA-13): AST walk over `platform/src/modules/lanzadera/` and `platform/src/`; fails if `legacy_hash`, `verify_legacy`, `sha256`, `old_password`, `migrate_password` found; runs as normal pytest; **RED first**: add a dummy `legacy_hash = ...` line in a test file and assert it fails
- [ ] 6.2 Create `tests/lanzadera/auth/test_no_legacy_in_auth.py` (DA-13 global scope): same AST walk on full `platform/src/`; wrapper for defense-in-depth
...
- [ ] 6.9 Run full suite: `pytest --cov=platform --cov-fail-under=85 -p platform.pytest_plugin.coverage_gate`; assert CRITICAL_HELPERS at 100%; assert global ≥ 85%
...
- [ ] 7.5 Verify no `legacy_hash`, `legacy_password`, `sha256`, `verify_legacy`, `old_password`, `migrate_password` in `platform/src/` (DA-13 AST test green)
- [ ] 7.6 Final `make quality-report` clean; `ruff format --check` passes; `mypy platform/src/` passes with `enable_error_code = ["ignore-without-code"]`
