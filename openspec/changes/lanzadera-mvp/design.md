...
│   │   └── versions/
│   │       ├── 0001_core_schema.py
│   │       ├── 0002_expedientes_schema.py  # expedientes (Phase 0 legacy)
│   │       ├── 0003_seed_apps.py           # DA-7b — 20 apps from fixture
│   │       ├── 0004_seed_profiles.py        # DA-12 — 160 profiles, legacy_role_map
│   │       ├── 0005_seed_users.py           # D89 — password_hash=NULL, AES-256-GCM dni
│   │       ├── 0006_seed_assignments.py     # DA-12 — 622 assignments, SinAcceso rule
│   │       └── 0007_seed_audit.py           # DA-11 — 200 events, no telemetry
│   ├── tests/
...
│   │       ├── assignments/test_legacy_role_map.py
...
| DA-7 | Migraciones Alembic 0001-0007 con estrategia Expand and Contract (D82). La 0005 deja `password_hash = NULL` para los 156 usuarios (D89); no carga hashes legacy. La 0007 inserta eventos históricos sin telemetría (DA-11). Cada migración es aditiva; ninguna tira columnas legacy del backend autoritativo. DA-7b justifica el origen de las fixtures. | D82 prohíbe migraciones destructivas en una sola release. Plan de rollback del proposal: `DROP SCHEMA lanzadera CASCADE` deja el `.accdb` intacto. | Migración que invente hashes dummy o inserte `legacy_hash` — rechazado por DA-3. | D82, D55, D89, DA-7b |
| DA-7b | Las fixtures para las migraciones 0003-0007 se derivan de la documentación walkthrough existente (`walkthrough-G*.json`, `docs/03-aplicaciones/lanzadera/data-model.md`) mediante `scripts/generate_lanzadera_fixtures.py` (seed=42, determinista). Se descartan `.accdb` + pyodbc porque el binario de producción tiene contraseña y `pydantic` no maneja credentials de Access Driver. El generador produce: 20 apps, 156 users, 160 profiles, 622 assignments, 200 audit events. Post-UAT, la migración real del `.accdb` usará `scripts/migrate_from_access.py` con pyodbc cuando IT facilite el Access Driver (#625). | DA-7 justifica la estrategia Alembic; el `.accdb` con contraseña cierra la vía pyodbc directa. El generador es determinista (seed=42). | Acceso directo al `.accdb` con pyodbc sin password — el binario de producción lo requiere. Generar fixtures a mano — rechazado por no reproducibilidad y riesgo de drift. | DA-7, #625 |
...
Las siete migraciones siguen D82 (Expand and Contract). Cada fila declara qué crea, qué siembra o qué migra, y la decisión heredada que materializa. DA-7b justifica las fixtures derivadas de la documentación walkthrough (sin `.accdb` con contraseña). La 0005 deja `password_hash = NULL` para los 156 usuarios; la columna `legacy_hash` no existe.
...
| `0002_expedientes_schema` | Schema compartido para módulos expedientes (Phase 0 legacy). Ya existente en `main`. | — |
| `0003_seed_apps` | Siembra las 20 filas de `apps` desde `data/fixtures/lanzadera/apps.json` (DA-7b); cada fila lleva `deployment_topology` y `requires_office_presence` derivados de `EjecucionEnOficina`; `registration_status='active'`. NO migra `Pass`, `Comando`, `URLDIrectorioIconoAplicacion` ni ningún atributo de lanzamiento. | D52, D53, D85, D58, DA-7b, apps/spec.md §Seed 20 apps |
| `0004_seed_profiles` | Para cada app, siembra al menos un profile con `code='default'`; además crea los profiles de mapeo legacy (`ADMIN`, `CALIDAD`, `CALIDAD_AVISOS`, `TECNICO`, `ECONOMIA`, `SECRETARIA`, `SIN_ACCESO`) cuando la app los tenía en `TbUsuariosAplicacionesPermisos`. `capabilities` es un JSONB mínimo inicial; el catálogo concreto de capabilities lo cierra `sdd-tasks` (§Decisiones pendientes — gap de profiles/spec.md). | D22, D45, D46, D110, profiles/spec.md §Standard profile |
| `0005_seed_users` | Migra los 156 usuarios desde `data/fixtures/lanzadera/users.json` con `email = lower(input.email)`, `name`, `dni_encrypted = AES-256-GCM(input.dni, PLATFORM_SECRET_KEY)`; **`password_hash = NULL`** y **`status = 'password_reset_required'`** para todos. **No** crea columna `legacy_hash`. Tests en `tests/lanzadera/users/test_contract.py`. | D88, D89, DA-3, auth-core/spec.md §Seeded users start with reset required |
| `0006_seed_assignments` | Siembra 622 filas activas de `user_app_assignments` desde `data/fixtures/lanzadera/assignments.json`. La regla `SinAcceso` exclusivo se aplicó en la generación de la fixture (cortocircuito en `legacy_role_map.resolve_legacy_roles`); cada row lleva `profile_id` ya resuelto. `granted_by = NULL` para todas las filas seed. Tests en `tests/lanzadera/assignments/test_contract.py`. | D22, D42, D85, D102, D110, H11, DA-12, assignments/spec.md |
| `0007_seed_audit` | Inserta 200 eventos históricos desde `data/fixtures/lanzadera/audit_events.json`. **No** persiste SSID, BSSID, coordenadas, máquina ni IP (DA-11). Tipos: `auth.login.success` (141), `auth.login.failure` (25), `app.open` (34). Tests en `tests/lanzadera/audit/test_contract.py`. | D55, D27, D112, H4, DA-11, audit/spec.md §Seed audit from legacy |

El orden 0001 → 0007 es **aditivo**: ninguna tira columnas ni borra filas en el schema ya creado. El rollback del MVP es `DROP SCHEMA lanzadera CASCADE;` — el `.accdb` legacy permanece intacto (proposal §Plan de rollback). Para un hotfix destructivo posterior (Contract), se sigue el flujo de dos releases según D82.
...
| `tests/lanzadera/assignments/test_legacy_role_map.py` | Aplica la matriz legacy → `user_app_assignments` para los 32 casos (2^5 combinaciones de flags Sí/No × `SinAcceso` Sí/No); verifica la regla exclusiva de `SinAcceso`; verifica que `DEFAULT` aparece cuando todas son No/NULL; verifica que el resultado coincide con la cardinalidad esperada (≈622 filas). | D85, D102, H11, DA-12, assignments/spec.md. |
...
| R-10 (H11) | Regla `SinAcceso` exclusivo: si una fila legacy tiene `SinAcceso=Sí` y otros Sí, solo se crea `SIN_ACCESO`. | `legacy_role_map.py` declara el cortocircuito; `test_legacy_role_map.py` cubre los 32 casos. | DA-12 + test de cardinalidad. |
...
- Aplicar 0001_core_schema, 0002_expedientes_schema, 0003_seed_apps, 0004_seed_profiles, 0005_seed_users, 0006_seed_assignments, 0007_seed_audit. Cada migración en su propio PR ≤ 400 líneas.
...
| G-3 | Lista de campos de catálogo legacy que NO migran (`Pass`, `Comando`, `URLDIrectorioIconoAplicacion`, etc.). | apps/spec.md L82 (`##ABIERTO##`) | Cerrado en DA-7 + DA-7b (migración 0003): solo migran `id`, `name`, `short_code`, `deployment_topology`, `requires_office_presence`, `registration_status`. La lista completa de campos descartados se documenta en `docs/03-aplicaciones/lanzadera/data-model.md`. |
...
2. Las fixtures para las migraciones 0003-0007 se derivan de la documentación walkthrough (`walkthrough-G*.json`, `docs/03-aplicaciones/lanzadera/data-model.md`) mediante `scripts/generate_lanzadera_fixtures.py` (DA-7b). El `.accdb` de producción tiene contraseña; la migración real post-UAT usará `scripts/migrate_from_access.py` con `pyodbc` cuando IT facilite el access driver (gap #625).
