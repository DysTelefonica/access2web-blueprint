...
class Extractor:
    ...
    def run_once(
        ...
        for table in tables or self._backend.list_tables():
            ...
            watermark = self._watermark.get(table)
            for row in self._backend.read_table(table):
                ...
                rows_extracted += 1
                watermark = pk  # type: ignore[assignment]
    ...
    def _extract_pk(self, row: object) -> int:
        ...
        """
        try:
            raw_value: int | str = row[self._pk]  # type: ignore[assignment]
        except (KeyError, TypeError) as exc:
            row_repr = dict(row) if hasattr(row, "keys") else row
            raise KeyError(
                f"row missing primary-key column {self._pk!r}: {row_repr}"
            ) from exc
