...
- **W40 make DRY BASELINE permanent** (#482): la única BASELINE DRY que queda (`dup:110a87c35376`, occurrences=3) es field-shape coincidence en los dataclass fields de admin/app/user — cada entidad declara `id` + `name` + un tercer campo, y el AST shape coincide por casualidad estructural. El normaliser de check_dry reescribe los docstring constants a su tipo (`str`), lo que explica por qué bloques con docstrings distintos aparecen como duplicados estructuralmente. No hay fix viable que preserve cada field set distinto. W40 bumpea `target` del inalcanzable `0` al conteo actual `3`, y mueve `target_date` a `2030-01-01`. El ratchet queda permanente al shape actual: cualquier futura adición al grupo (occurrences > 3) sigue siendo rechazada, pero la coincidencia actual no expira el 2026-11-30.
