#!/usr/bin/env python3
# HARNESS-PROVENANCE: deterministic-quality-harness v1.2 — assets/scripts/check_crap.py
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.json:
            print(json.dumps({"gate": "crap", "status": "error", "detail": str(exc)}))
        else:
    ...
    if args.json:
        ...
        print(json.dumps(build_report(measurements, line_coverage, status), indent=2))
    else:
        for line in lines:
            print(line)
    return exit_code
