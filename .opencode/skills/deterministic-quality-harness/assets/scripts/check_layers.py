#!/usr/bin/env python3
# HARNESS-PROVENANCE: deterministic-quality-harness v1.2 — assets/scripts/check_layers.py
...
def build_report(violations: list[Violation], status: str) -> dict:
    return {
        "gate": "layers",
        "status": status,
        "indicators": {
        ...
        "ceilings": {"violations": 0, "violation_classes": 0},
        "findings": [
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        print(json.dumps(build_report(violations, "pass" if exit_code == 0 else "fail"), indent=2))
    else:
        for line in lines:
            print(line)
    return exit_code
