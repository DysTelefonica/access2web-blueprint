#!/usr/bin/env python3
# HARNESS-PROVENANCE: deterministic-quality-harness v1.2 — assets/scripts/check_dry.py
...
#: Keyed by the pipe-joined sorted file list of the group. Shrink-only.
...
def collect_groups(root: Path) -> list[CloneGroup]:
    ...
    for digest, entries in ordered:
        ...
        if len(fresh) < MIN_OCCURRENCES:
            continue  # already reported as part of a larger overlapping clone
        for occurrence in fresh:
            for line in range(occurrence.start_line, occurrence.end_line + 1):
                claimed.add((occurrence.file, line))
        occurrences = tuple(sorted(fresh, key=lambda item: (item.file, item.start_line)))
        groups.append(
            CloneGroup(
                key="dup:" + "|".join(sorted({item.file for item in occurrences})),
                digest=digest[:12],
                statements=MIN_STATEMENTS,
                occurrences=occurrences,
            )
        )
    return sorted(groups, key=lambda group: (group.key, group.digest))
...
def build_report(root: Path, groups: list[CloneGroup], status: str) -> dict:
